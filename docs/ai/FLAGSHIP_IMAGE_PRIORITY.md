...
| 12 | **Dhal** | fallback | ❌ generic fallback | ? (external) | YES |
| 39 | **Poutine** | fallback | ❌ generic fallback | ? (external) | YES |
| 149 | **Pozole** | client-only | ? | ? (external) | no |
...
1. **Dhal** (id 12) — Drop image into public/images/dhal.{webp,jpg}, OR fetch via Spoonacular tool
1. **Poutine** (id 39) — Drop image into public/images/poutine.{webp,jpg}, OR fetch via Spoonacular tool
1. **Pozole** (id 149) — Promote URL from content.js IMG → recipe-images.js, OR drop public/images/pozole.{webp,jpg}

...
Generated by `scripts/audit-images.mjs` on 2026-08-28.
...
|---|---|---|---|---|
| **1. Local image override** | 116 | Curated `<img>` from `public/images/<slug>.{webp,jpg}` | curated URL | ✓ same-origin asset, always reaches |
| **2. Mapped external image** | 126 | Spoonacular/Wikipedia `<img>` from `recipe-images.js` | mapped URL | mostly (URL liveness not asserted from sandbox) |
...
Total recipes: 274. Buckets 3 + 4 = **0 recipes that
...
| With local override (`public/images/<slug>.{jpg,webp}`) | 135 |
| SSR renders emoji (buckets 3 + 4) | 0 |
| SSR renders `<img>` (buckets 1 + 2) | 242 |
| Flagship recipes | 20 |

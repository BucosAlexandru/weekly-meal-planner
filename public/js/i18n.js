const i18n = {
  ro: {
    title: "Planificator Mese Săptămânal",
    header: "Planificator Mese & Lista de Cumpărături",
    weekdays: ['Luni','Marți','Miercuri','Joi','Vineri','Sâmbătă','Duminică'],
    msg: {
      checking: "Se verifică...",
      empty: "Introduceți adresa de email!",
      server_error: "Eroare server. Încercați din nou.",
      invalid: "Nu există acces valid pentru acest email.",
      not_found: "Nu există acces pentru acest email. Plătește întâi sau verifică adresa.",
      valid: "✅ Ai acces! Poți descărca PDF-ul nelimitat:"
    },
    "btn.generate": "Generează PDF",
    "export.title": "Descarcă PDF",
    "export.sub": "Planul tău, rețete și lista de cumpărături",
    "btn.pay": "Premium – €3/lună",
    "btn.autoMenu": "Generează meniu aleator",
    "budget.toggle": "Meniu de buget (criză financiară)",
    "btn.verify": "Verifică Email",
    "btn.switchAccount": "Schimbă contul",
    "btn.download": "Descarcă PDF",
    "col.day": "Ziua",
    "col.lunch": "Prânz (ingrediente)",
    "col.dinner": "Cină (ingrediente)",
    "pw.lunch": "Prânz",
    "pw.dinner": "Cină",
    "pw.ovDays": "Zile planificate",
    "pw.ovTime": "Timp mediu",
    "pw.ovCost": "Cost estimat",
    "pw.ovCuisines": "Bucătării",
    "pw.reroll": "Altă rețetă",
    "pw.remove": "Scoate masa",
    "pw.undo": "Anulează",
    "pw.clearPlan": "Golește planul",
    "pw.cleared": "Plan golit",
    "pw.regenerated": "Plan nou generat",
    "pw.fromCart": "rețete adăugate din selecția ta",
    "pw.removed": "scos",
    "pw.noMore": "Ai folosit toate rețetele din acest filtru",
    "pw.addLunch": "Adaugă prânzul",
    "pw.addDinner": "Adaugă cina",
    "pw.searchHint": "caută după rețetă sau ingredient",
    "pw.searchPh": "Caută rețetă sau ingredient…",
    "pw.noResults": "Niciun rezultat. Încearcă un ingredient: pui, orez, ouă…",
    "pw.contains": "conține {ing}",
    "pw.recommended": "Potrivite cu planul tău",
    "pw.quickCheap": "Rapide și ieftine",
    "pw.favs": "Favoritele tale",
    "pw.allRecipes": "Toate rețetele",
    "pw.alreadyIn": "deja în plan · {day}",
    "col.origin": "Țara",
    "col.ingredients": "Ingrediente",
    "howIsMade": "Cum se face:",
    shoppingList: "Lista de cumpărături",
    "shopping.teaser.desc": "Organizată automat pe categorii și gata de cumpărături în PDF.",
    "shopping.empty": "Adaugă mese pentru a genera lista",
    maxed: "Ai atins limita maximă de PDF-uri gratuite.<br>Plătește pentru a debloca descărcarea nelimitată!",
    "pdf.free.label": "Previzualizare gratuită — 2 zile din 7",
    "pdf.locked.title": "🔒 Zilele 3 – 7 sunt disponibile cu abonamentul Premium",
    "pdf.locked.sub": "Deblochează planul complet pentru toată săptămâna, inclusiv rețete, ingrediente și lista de cumpărături.",
    "pdf.locked.cta": "Obține acces complet →",
    "pdf.tempUnavailable": "Exportul PDF este temporar indisponibil pentru această limbă. Restul aplicației funcționează normal.",
    "mode.meal": "O masă", "mode.day": "O zi", "mode.week": "O săptămână",
    "mode.heading.meal": "Alege o masă", "mode.heading.day": "Planul de azi", "mode.heading.week": "Planul săptămânii",
    "mode.btn.meal": "✨ Surprinde-mă!", "mode.btn.day": "Generează ziua", "mode.btn.week": "Generează săptămâna",
    "mode.col.meal": "Masă sugerată",
    "today": "Azi",
    placeholderL: "ex: cartofi, ceapă",
    placeholderD: "ex: pui, orez",
    "banner.unlock": "Ai plătit deja? Deblochează descărcări nelimitate PDF",
    "input.email": "Introdu emailul de plată",
    "access.granted": "✅ Ai acces! Poți descărca PDF-ul nelimitat:",
    "feedback.header": "Trimite feedback rapid",
    "feedback.placeholder": "Scrie aici sugestii, probleme sau păreri...",
    "feedback.button": "Trimite feedback",
    "feedback.thanks": "Mulțumim pentru feedback! Emailul se va deschide pentru trimitere.",
    "feedback.error": "Te rugăm să scrii un mesaj.",
    "feedback.note": "Feedbackul ajunge direct la dezvoltator.",
    "msg.valid": "✅ Ai acces! Poți descărca PDF-ul nelimitat:",
    "costEstimate": "Cost estimat",
    "perWeek": "săptămână · 2 persoane",
    "estWeeklyCost": "Cost estimat săptămână",
    "hero.badge": "✨ Gratuit · Fără cont · 14 limbi",
    "hero.sub": "Planifică mesele pentru 7 zile, generează lista de cumpărături automat și descarcă PDF — tot gratuit.",
    "hero.social": "Folosit în 14 limbi în toată lumea 🌍",
    "hero.cta": "🥗 Creează Planul Meu Gratuit",
    "mealsFound": "mese găsite",
    "how.title": "Cum funcționează",
    "how.step1.title": "Adaugă sau generează mesele",
    "how.step1.desc": "Scrie ce vrei să mănânci sau apasă «Generează» pentru a completa automat săptămâna",
    "how.step2.title": "Lista de cumpărături — automată",
    "how.step2.desc": "Ingredientele sunt extrase și organizate automat din planul tău",
    "how.step3.title": "Descarcă planul ca PDF",
    "how.step3.desc": "Primești un plan frumos de tipărit, gratuit o dată pe zi"
  },
  en: {
    title: "Weekly Meal Planner",
    header: "Weekly Meal Planner & Shopping List",
    weekdays: ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'],
    msg: {
      checking: "Checking...",
      empty: "Please enter your email!",
      server_error: "Server error. Please try again.",
      invalid: "No valid access found for this email.",
      not_found: "No access found. Please pay first or check the email.",
      valid: "✅ Access granted! You can download unlimited PDFs:"
    },
    "btn.generate": "Generate PDF",
    "export.title": "Download PDF",
    "export.sub": "Your plan, recipes & shopping list",
    "btn.pay": "Premium – €3/month",
    "btn.autoMenu": "Generate random menu",
    "budget.toggle": "Budget menu (crisis mode)",
    "btn.verify": "Verify Email",
    "btn.switchAccount": "Switch account",
    "btn.download": "Download PDF",
    "col.day": "Day",
    "col.lunch": "Lunch (ingredients)",
    "col.dinner": "Dinner (ingredients)",
    "pw.lunch": "Lunch",
    "pw.dinner": "Dinner",
    "pw.ovDays": "Days planned",
    "pw.ovTime": "Avg. time",
    "pw.ovCost": "Est. cost",
    "pw.ovCuisines": "Cuisines",
    "pw.reroll": "Another recipe",
    "pw.remove": "Remove meal",
    "pw.undo": "Undo",
    "pw.clearPlan": "Clear plan",
    "pw.cleared": "Plan cleared",
    "pw.regenerated": "New plan generated",
    "pw.fromCart": "added from your picks",
    "pw.removed": "removed",
    "pw.noMore": "You've used all recipes in this filter",
    "pw.addLunch": "Add lunch",
    "pw.addDinner": "Add dinner",
    "pw.searchHint": "search by recipe or ingredient",
    "pw.searchPh": "Search recipe or ingredient…",
    "pw.noResults": "No results. Try an ingredient: chicken, rice, eggs…",
    "pw.contains": "contains {ing}",
    "pw.recommended": "Fits your plan",
    "pw.quickCheap": "Quick & cheap",
    "pw.favs": "Your favorites",
    "pw.allRecipes": "All recipes",
    "pw.alreadyIn": "already planned · {day}",
    "col.origin": "Country",
    "col.ingredients": "Ingredients",
    "howIsMade": "How it's made:",
    shoppingList: "Shopping List",
    "shopping.teaser.desc": "Automatically organized by category and ready to shop in the PDF.",
    "shopping.empty": "Add meals to generate your shopping list",
    maxed: "You have reached the maximum number of free PDFs.<br>Pay to unlock unlimited downloads!",
    "pdf.free.label": "Free preview — 2 of 7 days",
    "pdf.locked.title": "🔒 Days 3 – 7 are available with Premium",
    "pdf.locked.sub": "Unlock the full 7-day plan with recipes, ingredients and shopping list.",
    "pdf.locked.cta": "Get full access →",
    "pdf.tempUnavailable": "PDF export is temporarily unavailable for this language. Everything else keeps working.",
    "mode.meal": "One Meal", "mode.day": "One Day", "mode.week": "One Week",
    "mode.heading.meal": "Pick a meal", "mode.heading.day": "Today's plan", "mode.heading.week": "Weekly plan",
    "mode.btn.meal": "✨ Surprise me!", "mode.btn.day": "Generate day", "mode.btn.week": "Generate week",
    "mode.col.meal": "Suggested meal",
    "today": "Today",
    placeholderL: "e.g. potatoes, onion",
    placeholderD: "e.g. chicken, rice",
    "banner.unlock": "Already paid? Unlock unlimited PDF downloads",
    "input.email": "Enter your payment email",
    "access.granted": "✅ Access granted! You can download unlimited PDFs:",
    "feedback.header": "Send quick feedback",
    "feedback.placeholder": "Write suggestions, problems, or feedback here...",
    "feedback.button": "Send feedback",
    "feedback.thanks": "Thank you for your feedback! Your email will open for sending.",
    "feedback.error": "Please write a feedback message.",
    "feedback.note": "Your feedback goes directly to the developer.",
    "msg.valid": "✅ Access granted! You can now download unlimited PDFs:",
    "costEstimate": "Estimated cost",
    "perWeek": "week · 2 people",
    "estWeeklyCost": "Estimated weekly cost",
    "hero.badge": "✨ Free · No account · 14 languages",
    "hero.sub": "Plan meals for 7 days, auto-generate your shopping list, and download a PDF — completely free.",
    "hero.social": "Used in 14 languages worldwide 🌍",
    "hero.cta": "🥗 Create My Free Meal Plan",
    "mealsFound": "meals found",
    "how.title": "How it works",
    "how.step1.title": "Add or generate meals",
    "how.step1.desc": "Type your meals or click «Generate» to auto-fill the entire week",
    "how.step2.title": "Shopping list — automatic",
    "how.step2.desc": "Ingredients are organized automatically from your plan",
    "how.step3.title": "Download as PDF",
    "how.step3.desc": "Get a beautiful printable plan, free once per day"
  },
  es: {
    title: "Planificador Semanal de Comidas",
    header: "Planificador de Comidas y Lista de Compras",
    weekdays: ['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'],
    msg: {
      checking: "Comprobando...",
      empty: "¡Por favor ingresa tu email!",
      server_error: "Error del servidor. Inténtalo de nuevo.",
      invalid: "No se encontró acceso válido para este email.",
      not_found: "No se encontró acceso. Por favor paga primero o revisa el email.",
      valid: "✅ ¡Acceso concedido! Ahora puedes descargar PDFs ilimitados:"
    },
    "btn.generate": "Generar PDF",
    "export.title": "Descargar PDF",
    "export.sub": "Tu plan, recetas y lista de compras",
    "btn.pay": "Premium – €3/mes",
    "btn.autoMenu": "Generar menú aleatorio",
    "budget.toggle": "Menú económico (modo crisis)",
    "btn.verify": "Verificar Email",
    "btn.switchAccount": "Cambiar cuenta",
    "btn.download": "Descargar PDF",
    "col.day": "Día",
    "col.lunch": "Almuerzo (ingredientes)",
    "col.dinner": "Cena (ingredientes)",
    "pw.lunch": "Almuerzo",
    "pw.dinner": "Cena",
    "pw.ovDays": "Días planificados",
    "pw.ovTime": "Tiempo medio",
    "pw.ovCost": "Coste estimado",
    "pw.ovCuisines": "Cocinas",
    "pw.reroll": "Otra receta",
    "pw.remove": "Quitar comida",
    "pw.undo": "Deshacer",
    "pw.clearPlan": "Vaciar el plan",
    "pw.cleared": "Plan vaciado",
    "pw.regenerated": "Nuevo plan generado",
    "pw.fromCart": "añadidas de tu selección",
    "pw.removed": "quitado",
    "pw.noMore": "Has usado todas las recetas de este filtro",
    "pw.addLunch": "Añadir almuerzo",
    "pw.addDinner": "Añadir cena",
    "pw.searchHint": "busca por receta o ingrediente",
    "pw.searchPh": "Busca receta o ingrediente…",
    "pw.noResults": "Sin resultados. Prueba un ingrediente: pollo, arroz, huevos…",
    "pw.contains": "contiene {ing}",
    "pw.recommended": "Acordes a tu plan",
    "pw.quickCheap": "Rápidas y baratas",
    "pw.favs": "Tus favoritas",
    "pw.allRecipes": "Todas las recetas",
    "pw.alreadyIn": "ya en el plan · {day}",
    "col.origin": "País",
    "col.ingredients": "Ingredientes",
    "howIsMade": "Cómo se hace:",
    shoppingList: "Lista de Compras",
    "shopping.teaser.desc": "Organizada automáticamente por categorías y lista para comprar en el PDF.",
    "shopping.empty": "Añade comidas para generar tu lista",
    maxed: "¡Has alcanzado el máximo de PDFs gratuitos!<br>¡Paga para descargas ilimitadas!",
    "pdf.free.label": "Vista previa gratuita — 2 de 7 días",
    "pdf.locked.title": "🔒 Los días 3 – 7 están disponibles con Premium",
    "pdf.locked.sub": "Desbloquea el plan completo de 7 días con recetas, ingredientes y lista de compras.",
    "pdf.locked.cta": "Obtener acceso completo →",
    "pdf.tempUnavailable": "La exportación a PDF no está disponible temporalmente para este idioma. Todo lo demás sigue funcionando.",
    "mode.meal": "Una comida", "mode.day": "Un día", "mode.week": "Una semana",
    "mode.heading.meal": "Elige una comida", "mode.heading.day": "Plan de hoy", "mode.heading.week": "Plan semanal",
    "mode.btn.meal": "✨ ¡Sorpréndeme!", "mode.btn.day": "Generar día", "mode.btn.week": "Generar semana",
    "mode.col.meal": "Comida sugerida", "today": "Hoy",
    placeholderL: "ej: papas, cebolla",
    placeholderD: "ej: pollo, arroz",
    "banner.unlock": "¿Ya pagaste? ¡Desbloquea descargas ilimitadas de PDF!",
    "input.email": "Ingresa tu email de pago",
    "access.granted": "✅ ¡Acceso concedido! Puedes descargar PDFs ilimitados:",
    "feedback.header": "Enviar feedback rápido",
    "feedback.placeholder": "Escribe aquí sugerencias, problemas o comentarios...",
    "feedback.button": "Enviar feedback",
    "feedback.thanks": "¡Gracias por tu feedback! Se abrirá el correo para enviarlo.",
    "feedback.error": "Por favor, escribe un mensaje.",
    "feedback.note": "El feedback va directamente al desarrollador.",
    "msg.valid": "✅ ¡Acceso concedido! Ahora puedes descargar PDFs ilimitados:",
    "costEstimate": "Coste estimado",
    "perWeek": "semana · 2 personas",
    "estWeeklyCost": "Coste semanal estimado",
    "hero.badge": "Gratis · 14 idiomas · 100+ recetas",
    "hero.sub": "Planifica tu semana, genera listas de compras automáticamente y descarga PDFs — gratis, en cualquier idioma.",
    "hero.social": "Usado en 14 idiomas en todo el mundo 🌍",
    "hero.cta": "🥗 Generar Mi Plan Gratis",
    "mealsFound": "comidas encontradas",
    "how.title": "Cómo funciona",
    "how.step1.title": "Añade o genera tus comidas",
    "how.step1.desc": "Escribe tus comidas o haz clic en «Generar» para completar la semana automáticamente",
    "how.step2.title": "Lista de compras — automática",
    "how.step2.desc": "Los ingredientes se organizan automáticamente desde tu plan",
    "how.step3.title": "Descarga como PDF",
    "how.step3.desc": "Obtén un plan imprimible gratuito, una vez al día"
  },
  fr: {
    title: "Planificateur de Repas Hebdomadaire",
    header: "Planificateur de Repas & Liste de Courses",
    weekdays: ['Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi','Dimanche'],
    msg: {
      checking: "Vérification...",
      empty: "Veuillez entrer votre email !",
      server_error: "Erreur du serveur. Veuillez réessayer.",
      invalid: "Aucun accès valide trouvé pour cet email.",
      not_found: "Aucun accès trouvé. Veuillez d'abord payer ou vérifier l'email.",
      valid: "✅ Accès accordé ! Vous pouvez désormais télécharger des PDF en illimité :"
    },
    "btn.generate": "Générer PDF",
    "export.title": "Télécharger PDF",
    "export.sub": "Votre plan, recettes et liste de courses",
    "btn.pay": "Premium – €3/mois",
    "btn.autoMenu": "Générer un menu aléatoire",
    "budget.toggle": "Menu économique (mode crise)",
    "btn.verify": "Vérifier l'email",
    "btn.switchAccount": "Changer de compte",
    "btn.download": "Télécharger le PDF",
    "col.day": "Jour",
    "col.lunch": "Déjeuner (ingrédients)",
    "col.dinner": "Dîner (ingrédients)",
    "pw.lunch": "Déjeuner",
    "pw.dinner": "Dîner",
    "pw.ovDays": "Jours planifiés",
    "pw.ovTime": "Temps moyen",
    "pw.ovCost": "Coût estimé",
    "pw.ovCuisines": "Cuisines",
    "pw.reroll": "Autre recette",
    "pw.remove": "Retirer le repas",
    "pw.undo": "Annuler",
    "pw.clearPlan": "Vider le plan",
    "pw.cleared": "Plan vidé",
    "pw.regenerated": "Nouveau plan généré",
    "pw.fromCart": "ajoutées de ta sélection",
    "pw.removed": "retiré",
    "pw.noMore": "Toutes les recettes de ce filtre sont déjà utilisées",
    "pw.addLunch": "Ajouter le déjeuner",
    "pw.addDinner": "Ajouter le dîner",
    "pw.searchHint": "cherchez par recette ou ingrédient",
    "pw.searchPh": "Recette ou ingrédient…",
    "pw.noResults": "Aucun résultat. Essayez un ingrédient : poulet, riz, œufs…",
    "pw.contains": "contient {ing}",
    "pw.recommended": "Assorties à votre plan",
    "pw.quickCheap": "Rapides et pas chères",
    "pw.favs": "Tes favorites",
    "pw.allRecipes": "Toutes les recettes",
    "pw.alreadyIn": "déjà au plan · {day}",
    "col.origin": "Pays",
    "col.ingredients": "Ingrédients",
    "howIsMade": "Comment c'est fait :",
    shoppingList: "Liste de Courses",
    "shopping.teaser.desc": "Organisée automatiquement par catégories et prête pour les courses dans le PDF.",
    "shopping.empty": "Ajoutez des repas pour générer votre liste",
    maxed: "Vous avez atteint la limite de PDFs gratuits.<br>Payer pour débloquer les téléchargements illimités !",
    "pdf.free.label": "Aperçu gratuit — 2 jours sur 7",
    "pdf.locked.title": "🔒 Les jours 3 – 7 sont disponibles avec Premium",
    "pdf.locked.sub": "Débloquez le plan complet de 7 jours avec recettes, ingrédients et liste de courses.",
    "pdf.locked.cta": "Obtenir l'accès complet →",
    "pdf.tempUnavailable": "L'export PDF est temporairement indisponible pour cette langue. Tout le reste fonctionne normalement.",
    "mode.meal": "Un repas", "mode.day": "Un jour", "mode.week": "Une semaine",
    "mode.heading.meal": "Choisir un repas", "mode.heading.day": "Plan du jour", "mode.heading.week": "Plan de la semaine",
    "mode.btn.meal": "✨ Surprends-moi !", "mode.btn.day": "Générer la journée", "mode.btn.week": "Générer la semaine",
    "mode.col.meal": "Repas suggéré", "today": "Aujourd'hui",
    placeholderL: "ex : pommes de terre, oignon",
    placeholderD: "ex : poulet, riz",
    "banner.unlock": "Déjà payé ? Débloquez les téléchargements PDF illimités",
    "input.email": "Entrez votre email de paiement",
    "access.granted": "✅ Accès accordé ! Vous pouvez télécharger des PDF illimités :",
    "feedback.header": "Envoyer un retour rapide",
    "feedback.placeholder": "Écrivez ici vos suggestions, problèmes ou avis...",
    "feedback.button": "Envoyer le feedback",
    "feedback.thanks": "Merci pour votre retour ! L’email va s’ouvrir pour l’envoi.",
    "feedback.error": "Veuillez écrire un message.",
    "feedback.note": "Le feedback va directement au développeur.",
    "msg.valid": "✅ Accès accordé ! Vous pouvez désormais télécharger des PDF en illimité :",
    "costEstimate": "Coût estimé",
    "perWeek": "semaine · 2 personnes",
    "estWeeklyCost": "Coût hebdomadaire estimé",
    "hero.badge": "Gratuit · 14 langues · 100+ recettes",
    "hero.sub": "Planifiez votre semaine, générez des listes de courses automatiquement et téléchargez des PDFs — gratuit, dans toute langue.",
    "hero.social": "Utilisé dans 14 langues dans le monde entier 🌍",
    "hero.cta": "🥗 Générer Mon Plan Gratuit",
    "mealsFound": "repas trouvés",
    "how.title": "Comment ça marche",
    "how.step1.title": "Ajoutez ou générez vos repas",
    "how.step1.desc": "Tapez vos repas ou cliquez sur «Générer» pour remplir automatiquement la semaine",
    "how.step2.title": "Liste de courses — automatique",
    "how.step2.desc": "Les ingrédients sont organisés automatiquement depuis votre plan",
    "how.step3.title": "Téléchargez en PDF",
    "how.step3.desc": "Obtenez un plan imprimible gratuit, une fois par jour"
  },
  ru: {
    title: "Еженедельный планировщик питания",
    header: "Планировщик питания & Список покупок",
    weekdays: ['Понедельник','Вторник','Среда','Четверг','Пятница','Суббота','Воскресенье'],
    msg: {
      checking: "Проверка...",
      empty: "Пожалуйста, введите вашу электронную почту!",
      server_error: "Ошибка сервера. Попробуйте еще раз.",
      invalid: "Нет доступа для этого email.",
      not_found: "Доступ не найден. Сначала оплатите или проверьте email.",
      valid: "✅ Доступ получен! Теперь вы можете скачивать неограниченное количество PDF:"
    },
    "btn.generate": "Создать PDF",
    "export.title": "Скачать PDF",
    "export.sub": "Ваш план, рецепты и список покупок",
    "btn.pay": "Премиум – €3/мес",
    "btn.autoMenu": "Сгенерировать случайное меню",
    "budget.toggle": "Бюджетное меню (режим кризиса)",
    "btn.verify": "Проверить Email",
    "btn.switchAccount": "Изменить аккаунт",
    "btn.download": "Скачать PDF",
    "col.day": "День",
    "col.lunch": "Обед (ингредиенты)",
    "col.dinner": "Ужин (ингредиенты)",
    "pw.lunch": "Обед",
    "pw.dinner": "Ужин",
    "pw.ovDays": "Дней запланировано",
    "pw.ovTime": "Среднее время",
    "pw.ovCost": "Оценочная стоимость",
    "pw.ovCuisines": "Кухни",
    "pw.reroll": "Другой рецепт",
    "pw.remove": "Убрать блюдо",
    "pw.undo": "Отменить",
    "pw.clearPlan": "Очистить план",
    "pw.cleared": "План очищен",
    "pw.regenerated": "Новый план создан",
    "pw.fromCart": "добавлено из подборки",
    "pw.removed": "убрано",
    "pw.noMore": "Все рецепты этого фильтра уже использованы",
    "pw.addLunch": "Добавить обед",
    "pw.addDinner": "Добавить ужин",
    "pw.searchHint": "поиск по рецепту или ингредиенту",
    "pw.searchPh": "Рецепт или ингредиент…",
    "pw.noResults": "Ничего не найдено. Попробуйте ингредиент: курица, рис, яйца…",
    "pw.contains": "содержит {ing}",
    "pw.recommended": "Подходят к вашему плану",
    "pw.quickCheap": "Быстро и недорого",
    "pw.favs": "Ваши избранные",
    "pw.allRecipes": "Все рецепты",
    "pw.alreadyIn": "уже в плане · {day}",
    "col.origin": "Страна",
    "col.ingredients": "Ингредиенты",
    "howIsMade": "Как готовится:",
    shoppingList: "Список покупок",
    "shopping.teaser.desc": "Автоматически сгруппирован по категориям и готов к покупкам в PDF.",
    "shopping.empty": "Добавьте блюда для создания списка",
    maxed: "Достигнут лимит бесплатных PDF.<br>Оплатите, чтобы разблокировать неограниченные загрузки!",
    "pdf.free.label": "Бесплатный просмотр — 2 из 7 дней",
    "pdf.locked.title": "🔒 Дни 3 – 7 доступны с Premium",
    "pdf.locked.sub": "Разблокируйте полный 7-дневный план с рецептами, ингредиентами и списком покупок.",
    "pdf.locked.cta": "Получить полный доступ →",
    "pdf.tempUnavailable": "Экспорт в PDF временно недоступен для этого языка. Всё остальное работает как обычно.",
    "mode.meal": "Одно блюдо", "mode.day": "Один день", "mode.week": "Одна неделя",
    "mode.heading.meal": "Выбрать блюдо", "mode.heading.day": "План на сегодня", "mode.heading.week": "План на неделю",
    "mode.btn.meal": "✨ Удиви меня!", "mode.btn.day": "Сгенерировать день", "mode.btn.week": "Сгенерировать неделю",
    "mode.col.meal": "Предлагаемое блюдо", "today": "Сегодня",
    placeholderL: "напр.: картофель, лук",
    placeholderD: "напр.: курица, рис",
    "banner.unlock": "Уже оплатили? Откройте неограниченные загрузки PDF",
    "input.email": "Введите ваш платежный email",
    "access.granted": "✅ Доступ предоставлен! Вы можете скачивать неограниченно PDF:",
    "feedback.header": "Отправить отзыв",
    "feedback.placeholder": "Напишите здесь предложения, проблемы или замечания...",
    "feedback.button": "Отправить отзыв",
    "feedback.thanks": "Спасибо за ваш отзыв! Откроется окно электронной почты.",
    "feedback.error": "Пожалуйста, напишите сообщение.",
    "feedback.note": "Отзыв поступает напрямую разработчику.",
    "msg.valid": "✅ Доступ получен! Теперь вы можете скачивать неограниченное количество PDF:",
    "costEstimate": "Оценочная стоимость",
    "perWeek": "неделя · 2 человека",
    "estWeeklyCost": "Оценочная стоимость недели",
    "hero.badge": "Бесплатно · 14 языков · 100+ рецептов",
    "hero.sub": "Планируйте неделю, автоматически создавайте списки покупок и скачивайте PDF — бесплатно, на любом языке.",
    "hero.social": "Используется на 14 языках по всему миру 🌍",
    "hero.cta": "🥗 Создать Мой Бесплатный План",
    "mealsFound": "блюд найдено",
    "how.title": "Как это работает",
    "how.step1.title": "Добавьте или сгенерируйте блюда",
    "how.step1.desc": "Введите блюда или нажмите «Создать», чтобы автоматически заполнить неделю",
    "how.step2.title": "Список покупок — автоматически",
    "how.step2.desc": "Ингредиенты организуются автоматически из вашего плана",
    "how.step3.title": "Скачайте в PDF",
    "how.step3.desc": "Получите красивый план для печати, бесплатно раз в день"
  },
  zh: {
    title: "每周用餐计划器",
    header: "用餐计划器 & 购物清单",
    weekdays: ['星期一','星期二','星期三','星期四','星期五','星期六','星期日'],
    msg: {
      checking: "正在检查...",
      empty: "请输入您的电子邮件！",
      server_error: "服务器错误，请重试。",
      invalid: "没有找到有效的访问权限。",
      not_found: "没有访问权限，请先付款或检查电子邮件。",
      valid: "✅ 访问已授权！您现在可以无限下载 PDF 文件："
    },
    "btn.generate": "生成 PDF",
    "export.title": "下载 PDF",
    "export.sub": "您的计划、食谱和购物清单",
    "btn.pay": "高级版 – €3/月",
    "btn.autoMenu": "生成随机菜单",
    "budget.toggle": "省钱菜单（危机模式）",
    "btn.verify": "验证邮箱",
    "btn.switchAccount": "切换账户",
    "btn.download": "下载 PDF",
    "col.day": "日期",
    "col.lunch": "午餐（食材）",
    "col.dinner": "晚餐（食材）",
    "pw.lunch": "午餐",
    "pw.dinner": "晚餐",
    "pw.ovDays": "已计划天数",
    "pw.ovTime": "平均时间",
    "pw.ovCost": "预估费用",
    "pw.ovCuisines": "菜系",
    "pw.reroll": "换一道菜",
    "pw.remove": "移除这餐",
    "pw.undo": "撤销",
    "pw.clearPlan": "清空计划",
    "pw.cleared": "计划已清空",
    "pw.regenerated": "已生成新计划",
    "pw.fromCart": "道菜已从收藏加入",
    "pw.removed": "已移除",
    "pw.noMore": "该筛选下的菜谱已全部使用",
    "pw.addLunch": "添加午餐",
    "pw.addDinner": "添加晚餐",
    "pw.searchHint": "按菜谱或食材搜索",
    "pw.searchPh": "搜索菜谱或食材…",
    "pw.noResults": "没有结果。试试食材：鸡肉、米饭、鸡蛋…",
    "pw.contains": "含{ing}",
    "pw.recommended": "与你的计划匹配",
    "pw.quickCheap": "快捷又实惠",
    "pw.favs": "你的收藏",
    "pw.allRecipes": "全部菜谱",
    "pw.alreadyIn": "已在计划中 · {day}",
    "col.origin": "国家",
    "col.ingredients": "配料",
    "howIsMade": "做法：",
    shoppingList: "购物清单",
    "shopping.teaser.desc": "自动按类别整理，在 PDF 中即可直接采购。",
    "shopping.empty": "添加餐食以生成清单",
    maxed: "已达到免费PDF上限。<br>付费可无限下载！",
    "pdf.free.label": "免费预览 — 7天中的2天",
    "pdf.locked.title": "🔒 第3至7天可通过Premium解锁",
    "pdf.locked.sub": "解锁完整7天计划，含食谱、食材和购物清单。",
    "pdf.locked.cta": "获取完整访问权限 →",
    "pdf.tempUnavailable": "该语言的 PDF 导出暂不可用。其他功能一切正常。",
    "mode.meal": "一顿饭", "mode.day": "一天", "mode.week": "一周",
    "mode.heading.meal": "选一道菜", "mode.heading.day": "今日计划", "mode.heading.week": "本周计划",
    "mode.btn.meal": "✨ 随机推荐！", "mode.btn.day": "生成今日菜单", "mode.btn.week": "生成本周菜单",
    "mode.col.meal": "推荐菜肴", "today": "今天",
    placeholderL: "例如：土豆，洋葱",
    placeholderD: "例如：鸡肉，米饭",
    "banner.unlock": "已经付款？解锁无限PDF下载",
    "input.email": "请输入您的付款邮箱",
    "access.granted": "✅ 已授权！你可以无限下载PDF：",
    "feedback.header": "快速反馈",
    "feedback.placeholder": "在这里写下建议、问题或反馈...",
    "feedback.button": "发送反馈",
    "feedback.thanks": "感谢您的反馈！将打开您的邮件发送。",
    "feedback.error": "请填写反馈信息。",
    "feedback.note": "反馈将直接发送给开发者。",
    "msg.valid": "✅ 访问已授权！您现在可以无限下载 PDF 文件：",
    "costEstimate": "预估费用",
    "perWeek": "周 · 2人",
    "estWeeklyCost": "每周预估费用",
    "hero.badge": "免费 · 14种语言 · 100+食谱",
    "hero.sub": "规划您的一周，自动生成购物清单，下载精美PDF——免费，支持任何语言。",
    "hero.social": "全球14种语言用户使用 🌍",
    "hero.cta": "🥗 生成我的免费餐饮计划",
    "mealsFound": "餐已找到",
    "how.title": "使用方法",
    "how.step1.title": "添加或生成餐食",
    "how.step1.desc": "输入您的餐食，或点击「生成」自动填充整周菜单",
    "how.step2.title": "购物清单 — 自动生成",
    "how.step2.desc": "食材从您的计划中自动整理",
    "how.step3.title": "下载为PDF",
    "how.step3.desc": "获取精美的可打印计划，每天免费一次"
  },
  ja: {
    title: "週間ミールプランナー",
    header: "ミールプランナー＆買い物リスト",
    weekdays: ['月曜日','火曜日','水曜日','木曜日','金曜日','土曜日','日曜日'],
    msg: {
      checking: "確認中...",
      empty: "メールアドレスを入力してください！",
      server_error: "サーバーエラー。もう一度お試しください。",
      invalid: "有効なアクセスが見つかりません。",
      not_found: "アクセスが見つかりません。先に支払うか、メールを確認してください。",
      valid: "✅ アクセス許可！無制限にPDFをダウンロードできます："
    },
    "btn.generate": "PDFを作成",
    "export.title": "PDFをダウンロード",
    "export.sub": "プラン、レシピ、ショッピングリスト",
    "btn.pay": "プレミアム – €3/月",
    "btn.autoMenu": "ランダムメニューを作成",
    "budget.toggle": "節約メニュー（危機モード）",
    "btn.verify": "メールを確認する",
    "btn.switchAccount": "アカウントを切り替える",
    "btn.download": "PDFをダウンロード",
    "col.day": "日付",
    "col.lunch": "昼食（材料）",
    "col.dinner": "夕食（材料）",
    "pw.lunch": "昼食",
    "pw.dinner": "夕食",
    "pw.ovDays": "計画済みの日数",
    "pw.ovTime": "平均時間",
    "pw.ovCost": "推定費用",
    "pw.ovCuisines": "料理の種類",
    "pw.reroll": "別のレシピ",
    "pw.remove": "この食事を外す",
    "pw.undo": "元に戻す",
    "pw.clearPlan": "プランをクリア",
    "pw.cleared": "プランをクリアしました",
    "pw.regenerated": "新しいプランを作成しました",
    "pw.fromCart": "品を選択から追加",
    "pw.removed": "を削除しました",
    "pw.noMore": "このフィルターのレシピはすべて使用済みです",
    "pw.addLunch": "ランチを追加",
    "pw.addDinner": "ディナーを追加",
    "pw.searchHint": "レシピや食材で検索",
    "pw.searchPh": "レシピ・食材を検索…",
    "pw.noResults": "見つかりません。食材で検索：鶏肉、米、卵…",
    "pw.contains": "{ing}入り",
    "pw.recommended": "プランに合うレシピ",
    "pw.quickCheap": "早くて安い",
    "pw.favs": "お気に入り",
    "pw.allRecipes": "すべてのレシピ",
    "pw.alreadyIn": "プランに追加済み · {day}",
    "col.origin": "国",
    "col.ingredients": "材料",
    "howIsMade": "作り方：",
    shoppingList: "買い物リスト",
    "shopping.teaser.desc": "カテゴリーごとに自動整理され、PDFですぐ買い物できます。",
    "shopping.empty": "食事を追加してリストを作成",
    maxed: "無料PDFの上限に達しました。<br>無制限ダウンロードは有料！",
    "pdf.free.label": "無料プレビュー — 7日中2日",
    "pdf.locked.title": "🔒 3〜7日目はPremiumで利用可能",
    "pdf.locked.sub": "レシピ・食材・買い物リスト付きの7日間フルプランを解除。",
    "pdf.locked.cta": "フルアクセスを取得 →",
    "pdf.tempUnavailable": "この言語のPDFエクスポートは一時的にご利用いただけません。その他の機能は通常どおりご利用いただけます。",
    "mode.meal": "1食分", "mode.day": "1日分", "mode.week": "1週間",
    "mode.heading.meal": "1食を選ぶ", "mode.heading.day": "今日のプラン", "mode.heading.week": "今週のプラン",
    "mode.btn.meal": "✨ サプライズ！", "mode.btn.day": "今日を生成", "mode.btn.week": "今週を生成",
    "mode.col.meal": "おすすめの料理", "today": "今日",
    placeholderL: "例：ジャガイモ、玉ねぎ",
    placeholderD: "例：鶏肉、ご飯",
    "banner.unlock": "すでに支払いましたか？無制限PDFダウンロードを解除",
    "input.email": "支払いメールを入力してください",
    "access.granted": "✅ アクセス許可！無制限にPDFをダウンロードできます：",
    "feedback.header": "フィードバックを送信",
    "feedback.placeholder": "ここにご意見や問題点などをご記入ください...",
    "feedback.button": "フィードバック送信",
    "feedback.thanks": "ご意見ありがとうございます！メールが開きます。",
    "feedback.error": "メッセージを入力してください。",
    "feedback.note": "フィードバックは開発者に直接届きます。",
    "msg.valid": "✅ アクセス許可！無制限にPDFをダウンロードできます：",
    "costEstimate": "推定費用",
    "perWeek": "週 · 2人",
    "estWeeklyCost": "週の推定費用",
    "hero.badge": "無料 · 14言語 · 100+レシピ",
    "hero.sub": "週の計画を立て、買い物リストを自動生成し、PDFをダウンロード — 無料、どの言語でも。",
    "hero.social": "世界14言語で使用 🌍",
    "hero.cta": "🥗 無料プランを作成する",
    "mealsFound": "食事が見つかりました",
    "how.title": "使い方",
    "how.step1.title": "食事を追加または自動生成",
    "how.step1.desc": "食事を入力するか「生成」をクリックして1週間を自動入力",
    "how.step2.title": "買い物リスト — 自動作成",
    "how.step2.desc": "食材はプランから自動的に整理されます",
    "how.step3.title": "PDFでダウンロード",
    "how.step3.desc": "美しい印刷用プランを1日1回無料で取得"
  },
  pt: {
    title: "Planejador Semanal de Refeições",
    header: "Planejador de Refeições & Lista de Compras",
    weekdays: ['Segunda-feira','Terça-feira','Quarta-feira','Quinta-feira','Sexta-feira','Sábado','Domingo'],
    msg: {
      checking: "Verificando...",
      empty: "Digite seu e-mail!",
      server_error: "Erro no servidor. Tente novamente.",
      invalid: "Nenhum acesso válido encontrado para este e-mail.",
      not_found: "Nenhum acesso encontrado. Pague primeiro ou verifique o e-mail.",
      valid: "✅ Acesso concedido! Agora você pode baixar PDFs ilimitados:"
    },
    "btn.generate": "Gerar PDF",
    "export.title": "Baixar PDF",
    "export.sub": "Seu plano, receitas e lista de compras",
    "btn.pay": "Premium – €3/mês",
    "btn.autoMenu": "Gerar menu aleatório",
    "budget.toggle": "Menu econômico (modo crise)",
    "btn.verify": "Verificar Email",
    "btn.switchAccount": "Trocar conta",
    "btn.download": "Baixar PDF",
    "col.day": "Dia",
    "col.lunch": "Almoço (ingredientes)",
    "col.dinner": "Jantar (ingredientes)",
    "pw.lunch": "Almoço",
    "pw.dinner": "Jantar",
    "pw.ovDays": "Dias planeados",
    "pw.ovTime": "Tempo médio",
    "pw.ovCost": "Custo estimado",
    "pw.ovCuisines": "Cozinhas",
    "pw.reroll": "Outra receita",
    "pw.remove": "Remover refeição",
    "pw.undo": "Anular",
    "pw.clearPlan": "Limpar o plano",
    "pw.cleared": "Plano limpo",
    "pw.regenerated": "Novo plano gerado",
    "pw.fromCart": "adicionadas da sua seleção",
    "pw.removed": "removido",
    "pw.noMore": "Já usou todas as receitas deste filtro",
    "pw.addLunch": "Adicionar almoço",
    "pw.addDinner": "Adicionar jantar",
    "pw.searchHint": "procure por receita ou ingrediente",
    "pw.searchPh": "Pesquisar receita ou ingrediente…",
    "pw.noResults": "Sem resultados. Tente um ingrediente: frango, arroz, ovos…",
    "pw.contains": "contém {ing}",
    "pw.recommended": "A condizer com o teu plano",
    "pw.quickCheap": "Rápidas e baratas",
    "pw.favs": "Suas favoritas",
    "pw.allRecipes": "Todas as receitas",
    "pw.alreadyIn": "já no plano · {day}",
    "col.origin": "País",
    "col.ingredients": "Ingredientes",
    "howIsMade": "Como se faz:",
    shoppingList: "Lista de Compras",
    "shopping.teaser.desc": "Organizada automaticamente por categorias e pronta para comprar no PDF.",
    "shopping.empty": "Adicione refeições para gerar sua lista",
    maxed: "Você atingiu o limite de PDFs gratuitos.<br>Pague para desbloquear downloads ilimitados!",
    "pdf.free.label": "Pré-visualização gratuita — 2 de 7 dias",
    "mode.meal": "Uma refeição", "mode.day": "Um dia", "mode.week": "Uma semana",
    "mode.heading.meal": "Escolher refeição", "mode.heading.day": "Plano de hoje", "mode.heading.week": "Plano semanal",
    "mode.btn.meal": "✨ Surpreenda-me!", "mode.btn.day": "Gerar dia", "mode.btn.week": "Gerar semana",
    "mode.col.meal": "Refeição sugerida", "today": "Hoje",
    "pdf.locked.title": "🔒 Os dias 3 – 7 estão disponíveis com Premium",
    "pdf.locked.sub": "Desbloqueie o plano completo de 7 dias com receitas, ingredientes e lista de compras.",
    "pdf.locked.cta": "Obter acesso completo →",
    "pdf.tempUnavailable": "A exportação de PDF está temporariamente indisponível para este idioma. Todo o resto continua a funcionar.",
    placeholderL: "ex: batata, cebola",
    placeholderD: "ex: frango, arroz",
    "banner.unlock": "Já pagou? Desbloqueie downloads ilimitados de PDF",
    "input.email": "Digite seu e-mail de pagamento",
    "access.granted": "✅ Acesso concedido! Você pode baixar PDFs ilimitados:",
    "feedback.header": "Enviar feedback rápido",
    "feedback.placeholder": "Escreva sugestões, problemas ou comentários aqui...",
    "feedback.button": "Enviar feedback",
    "feedback.thanks": "Obrigado pelo feedback! Seu e-mail será aberto para envio.",
    "feedback.error": "Por favor, escreva uma mensagem.",
    "feedback.note": "O feedback vai diretamente para o desenvolvedor.",
    "msg.valid": "✅ Acesso concedido! Agora você pode baixar PDFs ilimitados:",
    "costEstimate": "Custo estimado",
    "perWeek": "semana · 2 pessoas",
    "estWeeklyCost": "Custo semanal estimado",
    "hero.badge": "Gratuito · 14 idiomas · 100+ receitas",
    "hero.sub": "Planeje sua semana, gere listas de compras automaticamente e baixe PDFs — grátis, em qualquer idioma.",
    "hero.social": "Usado em 14 idiomas no mundo todo 🌍",
    "hero.cta": "🥗 Gerar Meu Plano Grátis",
    "mealsFound": "refeições encontradas",
    "how.title": "Como funciona",
    "how.step1.title": "Adicione ou gere suas refeições",
    "how.step1.desc": "Digite suas refeições ou clique em «Gerar» para preencher a semana automaticamente",
    "how.step2.title": "Lista de compras — automática",
    "how.step2.desc": "Os ingredientes são organizados automaticamente a partir do seu plano",
    "how.step3.title": "Baixe como PDF",
    "how.step3.desc": "Obtenha um plano imprimível gratuito, uma vez por dia"
  },
  de: {
    title: "Wöchentlicher Essensplaner",
    header: "Essensplaner & Einkaufsliste",
    weekdays: ['Montag','Dienstag','Mittwoch','Donnerstag','Freitag','Samstag','Sonntag'],
    msg: {
      checking: "Wird überprüft...",
      empty: "Bitte geben Sie Ihre E-Mail-Adresse ein!",
      server_error: "Serverfehler. Bitte versuchen Sie es erneut.",
      invalid: "Kein gültiger Zugang für diese E-Mail gefunden.",
      not_found: "Kein Zugang gefunden. Bitte zuerst bezahlen oder E-Mail prüfen.",
      valid: "✅ Zugriff gewährt! Du kannst nun unbegrenzt PDFs herunterladen:"
    },
    "btn.generate": "PDF erstellen",
    "export.title": "PDF herunterladen",
    "export.sub": "Ihr Plan, Rezepte & Einkaufsliste",
    "btn.pay": "Premium – €3/Monat",
    "btn.autoMenu": "Zufälliges Menü erstellen",
    "budget.toggle": "Budget-Menü (Krisenmodus)",
    "btn.verify": "E-Mail überprüfen",
    "btn.switchAccount": "Konto wechseln",
    "btn.download": "PDF herunterladen",
    "col.day": "Tag",
    "col.lunch": "Mittagessen (Zutaten)",
    "col.dinner": "Abendessen (Zutaten)",
    "pw.lunch": "Mittagessen",
    "pw.dinner": "Abendessen",
    "pw.ovDays": "Geplante Tage",
    "pw.ovTime": "Ø Zeit",
    "pw.ovCost": "Geschätzte Kosten",
    "pw.ovCuisines": "Küchen",
    "pw.reroll": "Anderes Rezept",
    "pw.remove": "Mahlzeit entfernen",
    "pw.undo": "Rückgängig",
    "pw.clearPlan": "Plan leeren",
    "pw.cleared": "Plan geleert",
    "pw.regenerated": "Neuer Plan erstellt",
    "pw.fromCart": "aus deiner Auswahl hinzugefügt",
    "pw.removed": "entfernt",
    "pw.noMore": "Alle Rezepte dieses Filters sind bereits verwendet",
    "pw.addLunch": "Mittagessen hinzufügen",
    "pw.addDinner": "Abendessen hinzufügen",
    "pw.searchHint": "nach Rezept oder Zutat suchen",
    "pw.searchPh": "Rezept oder Zutat suchen…",
    "pw.noResults": "Keine Treffer. Versuch eine Zutat: Huhn, Reis, Eier…",
    "pw.contains": "enthält {ing}",
    "pw.recommended": "Passend zu deinem Plan",
    "pw.quickCheap": "Schnell & günstig",
    "pw.favs": "Deine Favoriten",
    "pw.allRecipes": "Alle Rezepte",
    "pw.alreadyIn": "schon im Plan · {day}",
    "col.origin": "Land",
    "col.ingredients": "Zutaten",
    "howIsMade": "So wird's gemacht:",
    shoppingList: "Einkaufsliste",
    "shopping.teaser.desc": "Automatisch nach Kategorien sortiert und einkaufsfertig im PDF.",
    "shopping.empty": "Mahlzeiten hinzufügen, um die Liste zu erstellen",
    maxed: "Du hast das Limit kostenloser PDFs erreicht.<br>Zahle für unbegrenzte Downloads!",
    "pdf.free.label": "Kostenlose Vorschau — 2 von 7 Tagen",
    "mode.meal": "Eine Mahlzeit", "mode.day": "Ein Tag", "mode.week": "Eine Woche",
    "mode.heading.meal": "Mahlzeit wählen", "mode.heading.day": "Plan für heute", "mode.heading.week": "Wochenplan",
    "mode.btn.meal": "✨ Überrasch mich!", "mode.btn.day": "Tag generieren", "mode.btn.week": "Woche generieren",
    "mode.col.meal": "Vorgeschlagenes Gericht", "today": "Heute",
    "pdf.locked.title": "🔒 Tage 3 – 7 sind mit Premium verfügbar",
    "pdf.locked.sub": "Schalte den vollständigen 7-Tage-Plan mit Rezepten, Zutaten und Einkaufsliste frei.",
    "pdf.locked.cta": "Vollen Zugang erhalten →",
    "pdf.tempUnavailable": "Der PDF-Export ist für diese Sprache vorübergehend nicht verfügbar. Alles andere funktioniert weiterhin.",
    placeholderL: "z.B.: Kartoffeln, Zwiebeln",
    placeholderD: "z.B.: Hähnchen, Reis",
    "banner.unlock": "Schon bezahlt? Entsperre unbegrenzte PDF-Downloads",
    "input.email": "Geben Sie Ihre Zahlungs-E-Mail ein",
    "access.granted": "✅ Zugang gewährt! Sie können unbegrenzt PDFs herunterladen:",
    "feedback.header": "Schnelles Feedback senden",
    "feedback.placeholder": "Schreiben Sie hier Ihre Vorschläge, Probleme oder Meinungen...",
    "feedback.button": "Feedback senden",
    "feedback.thanks": "Danke für Ihr Feedback! Ihr E-Mail-Programm öffnet sich.",
    "feedback.error": "Bitte geben Sie eine Nachricht ein.",
    "feedback.note": "Ihr Feedback geht direkt an den Entwickler.",
    "msg.valid": "✅ Zugriff gewährt! Du kannst nun unbegrenzt PDFs herunterladen:",
    "costEstimate": "Geschätzte Kosten",
    "perWeek": "Woche · 2 Personen",
    "estWeeklyCost": "Geschätzte Wochenkosten",
    "hero.badge": "Kostenlos · 14 Sprachen · 100+ Rezepte",
    "hero.sub": "Planen Sie Ihre Woche, erstellen Sie automatisch Einkaufslisten und laden Sie PDFs herunter — kostenlos, in jeder Sprache.",
    "hero.social": "In 14 Sprachen weltweit genutzt 🌍",
    "hero.cta": "🥗 Meinen kostenlosen Plan erstellen",
    "mealsFound": "Mahlzeiten gefunden",
    "how.title": "So funktioniert es",
    "how.step1.title": "Mahlzeiten hinzufügen oder generieren",
    "how.step1.desc": "Tragen Sie Mahlzeiten ein oder klicken Sie auf «Generieren», um die Woche automatisch auszufüllen",
    "how.step2.title": "Einkaufsliste — automatisch",
    "how.step2.desc": "Zutaten werden automatisch aus Ihrem Plan zusammengestellt",
    "how.step3.title": "Als PDF herunterladen",
    "how.step3.desc": "Erhalten Sie einen schönen druckbaren Plan, einmal täglich kostenlos"
  },
  ar: {
    title: "مخطط وجبات أسبوعي",
    header: "مخطط الوجبات وقائمة التسوق",
    weekdays: ['الاثنين','الثلاثاء','الأربعاء','الخميس','الجمعة','السبت','الأحد'],
    msg: {
      checking: "جاري التحقق...",
      empty: "يرجى إدخال بريدك الإلكتروني!",
      server_error: "خطأ في الخادم. حاول مرة أخرى.",
      invalid: "لا يوجد وصول صالح لهذا البريد الإلكتروني.",
      not_found: "لا يوجد وصول. يرجى الدفع أولاً أو التحقق من البريد الإلكتروني.",
      valid: "✅ تم منح الوصول! يمكنك الآن تنزيل ملفات PDF غير محدودة:"
    },
    "btn.generate": "إنشاء PDF",
    "export.title": "تحميل PDF",
    "export.sub": "خطتك ووصفاتك وقائمة التسوق",
    "btn.pay": "بريميوم – €3/شهر",
    "btn.autoMenu": "إنشاء قائمة عشوائية",
    "budget.toggle": "قائمة اقتصادية (وضع الأزمة)",
    "btn.verify": "تحقق من البريد الإلكتروني",
    "btn.switchAccount": "تبديل الحساب",
    "btn.download": "تحميل PDF",
    "col.day": "اليوم",
    "col.lunch": "الغداء (المكونات)",
    "col.dinner": "العشاء (المكونات)",
    "pw.lunch": "الغداء",
    "pw.dinner": "العشاء",
    "pw.ovDays": "أيام مخططة",
    "pw.ovTime": "متوسط الوقت",
    "pw.ovCost": "التكلفة المقدرة",
    "pw.ovCuisines": "مطابخ",
    "pw.reroll": "وصفة أخرى",
    "pw.remove": "إزالة الوجبة",
    "pw.undo": "تراجع",
    "pw.clearPlan": "مسح الخطة",
    "pw.cleared": "تم مسح الخطة",
    "pw.regenerated": "تم إنشاء خطة جديدة",
    "pw.fromCart": "أُضيفت من اختيارك",
    "pw.removed": "أُزيلت",
    "pw.noMore": "استخدمت كل وصفات هذا الفلتر",
    "pw.addLunch": "أضف الغداء",
    "pw.addDinner": "أضف العشاء",
    "pw.searchHint": "ابحث حسب الوصفة أو المكوّن",
    "pw.searchPh": "ابحث عن وصفة أو مكوّن…",
    "pw.noResults": "لا نتائج. جرّب مكوّناً: دجاج، أرز، بيض…",
    "pw.contains": "يحتوي على {ing}",
    "pw.recommended": "مناسبة لخطتك",
    "pw.quickCheap": "سريعة ورخيصة",
    "pw.favs": "المفضلة لديك",
    "pw.allRecipes": "كل الوصفات",
    "pw.alreadyIn": "موجودة في الخطة · {day}",
    "col.origin": "الدولة",
    "col.ingredients": "المكونات",
    "howIsMade": "طريقة التحضير:",
    shoppingList: "قائمة التسوق",
    "shopping.teaser.desc": "منظَّمة تلقائيًا حسب الفئات وجاهزة للتسوق في PDF.",
    "shopping.empty": "أضف وجبات لإنشاء قائمتك",
    maxed: "لقد وصلت إلى الحد الأقصى من ملفات PDF المجانية.<br>ادفع لتحميل غير محدود!",
    "pdf.free.label": "معاينة مجانية — يومان من أصل 7",
    "mode.meal": "وجبة واحدة", "mode.day": "يوم واحد", "mode.week": "أسبوع واحد",
    "mode.heading.meal": "اختر وجبة", "mode.heading.day": "خطة اليوم", "mode.heading.week": "خطة الأسبوع",
    "mode.btn.meal": "✨ فاجئني!", "mode.btn.day": "توليد اليوم", "mode.btn.week": "توليد الأسبوع",
    "mode.col.meal": "الوجبة المقترحة", "today": "اليوم",
    "pdf.locked.title": "🔒 الأيام 3 – 7 متاحة مع Premium",
    "pdf.locked.sub": "افتح الخطة الكاملة لـ 7 أيام مع الوصفات والمكونات وقائمة التسوق.",
    "pdf.locked.cta": "احصل على الوصول الكامل →",
    "pdf.tempUnavailable": "تصدير PDF غير متاح مؤقتًا لهذه اللغة — نعمل على إصلاح ذلك. جميع الميزات الأخرى تعمل كالمعتاد.",
    placeholderL: "مثال: بطاطس، بصل",
    placeholderD: "مثال: دجاج، أرز",
    "banner.unlock": "هل دفعت بالفعل؟ افتح تنزيلات PDF غير المحدودة",
    "input.email": "أدخل بريدك الإلكتروني للدفع",
    "access.granted": "✅ لديك حق الوصول! يمكنك تحميل PDF بلا حدود:",
    "feedback.header": "أرسل ملاحظات سريعة",
    "feedback.placeholder": "اكتب هنا اقتراحاتك أو مشاكلك أو رأيك...",
    "feedback.button": "إرسال الملاحظات",
    "feedback.thanks": "شكرًا لملاحظاتك! سيتم فتح البريد الإلكتروني للإرسال.",
    "feedback.error": "يرجى كتابة رسالة.",
    "feedback.note": "الملاحظات تصل مباشرة إلى المطور.",
    "msg.valid": "✅ تم منح الوصول! يمكنك الآن تنزيل ملفات PDF غير محدودة:",
    "costEstimate": "التكلفة المقدرة",
    "perWeek": "أسبوع · شخصان",
    "estWeeklyCost": "التكلفة الأسبوعية المقدرة",
    "hero.badge": "مجاني · 14 لغة · أكثر من 100 وصفة",
    "hero.sub": "خطط لأسبوعك، أنشئ قوائم التسوق تلقائيًا وحمّل ملفات PDF — مجانًا، بأي لغة.",
    "hero.social": "مستخدم بـ 14 لغة حول العالم 🌍",
    "hero.cta": "🥗 إنشاء خطتي المجانية",
    "mealsFound": "وجبات موجودة",
    "how.title": "كيف يعمل",
    "how.step1.title": "أضف أو أنشئ وجباتك",
    "how.step1.desc": "اكتب وجباتك أو انقر على «توليد» لملء الأسبوع تلقائيًا",
    "how.step2.title": "قائمة التسوق — تلقائية",
    "how.step2.desc": "يتم تنظيم المكونات تلقائيًا من خطتك",
    "how.step3.title": "تحميل كـ PDF",
    "how.step3.desc": "احصل على خطة جميلة للطباعة، مجانًا مرة واحدة يوميًا"
  },
  hi: {
    title: "साप्ताहिक भोजन योजना",
    header: "भोजन योजनाकार और खरीदारी सूची",
    weekdays: ['सोमवार','मंगलवार','बुधवार','गुरुवार','शुक्रवार','शनिवार','रविवार'],
    msg: {
      checking: "जांच हो रही है...",
      empty: "कृपया अपना ईमेल दर्ज करें!",
      server_error: "सर्वर त्रुटि। कृपया पुनः प्रयास करें।",
      invalid: "इस ईमेल के लिए कोई वैध एक्सेस नहीं मिला।",
      not_found: "कोई एक्सेस नहीं मिला। पहले भुगतान करें या ईमेल जांचें।",
      valid: "✅ पहुँच प्राप्त! आप अनलिमिटेड PDF डाउनलोड कर सकते हैं:"
    },
    "btn.generate": "PDF बनाएँ",
    "export.title": "PDF डाउनलोड करें",
    "export.sub": "आपकी योजना, व्यंजन और खरीदारी सूची",
    "btn.pay": "प्रीमियम – €3/माह",
    "btn.autoMenu": "रैंडम मेनू बनाएं",
    "budget.toggle": "कम बजट मेनू (क्राइसिस मोड)",
    "btn.verify": "ईमेल सत्यापित करें",
    "btn.switchAccount": "खाता बदलें",
    "btn.download": "PDF डाउनलोड करें",
    "col.day": "दिन",
    "col.lunch": "दोपहर का भोजन (सामग्री)",
    "col.dinner": "रात का खाना (सामग्री)",
    "pw.lunch": "दोपहर का भोजन",
    "pw.dinner": "रात का खाना",
    "pw.ovDays": "नियोजित दिन",
    "pw.ovTime": "औसत समय",
    "pw.ovCost": "अनुमानित लागत",
    "pw.ovCuisines": "व्यंजन शैलियाँ",
    "pw.reroll": "दूसरी रेसिपी",
    "pw.remove": "भोजन हटाएँ",
    "pw.undo": "पूर्ववत करें",
    "pw.clearPlan": "योजना खाली करें",
    "pw.cleared": "योजना खाली की गई",
    "pw.regenerated": "नई योजना बनाई गई",
    "pw.fromCart": "आपकी पसंद से जोड़ी गईं",
    "pw.removed": "हटाया गया",
    "pw.noMore": "इस फ़िल्टर की सभी रेसिपी उपयोग हो चुकी हैं",
    "pw.addLunch": "दोपहर का खाना जोड़ें",
    "pw.addDinner": "रात का खाना जोड़ें",
    "pw.searchHint": "रेसिपी या सामग्री से खोजें",
    "pw.searchPh": "रेसिपी या सामग्री खोजें…",
    "pw.noResults": "कोई परिणाम नहीं। कोई सामग्री आज़माएँ: चिकन, चावल, अंडे…",
    "pw.contains": "{ing} शामिल",
    "pw.recommended": "आपकी योजना से मेल खाती",
    "pw.quickCheap": "झटपट और सस्ती",
    "pw.favs": "आपकी पसंदीदा",
    "pw.allRecipes": "सभी रेसिपी",
    "pw.alreadyIn": "योजना में पहले से · {day}",
    "col.origin": "देश",
    "col.ingredients": "सामग्री",
    "howIsMade": "कैसे बनाते हैं:",
    shoppingList: "खरीदारी की सूची",
    "shopping.teaser.desc": "श्रेणियों के अनुसार स्वचालित रूप से व्यवस्थित और PDF में खरीदारी के लिए तैयार।",
    "shopping.empty": "सूची बनाने के लिए भोजन जोड़ें",
    maxed: "आपने मुफ्त PDF की अधिकतम सीमा तक पहुँच गए हैं।<br>अनलिमिटेड डाउनलोड के लिए भुगतान करें!",
    "pdf.free.label": "मुफ्त पूर्वावलोकन — 7 में से 2 दिन",
    "mode.meal": "एक भोजन", "mode.day": "एक दिन", "mode.week": "एक सप्ताह",
    "mode.heading.meal": "एक भोजन चुनें", "mode.heading.day": "आज की योजना", "mode.heading.week": "साप्ताहिक योजना",
    "mode.btn.meal": "✨ मुझे चौंकाओ!", "mode.btn.day": "दिन बनाएं", "mode.btn.week": "सप्ताह बनाएं",
    "mode.col.meal": "सुझाया गया भोजन", "today": "आज",
    "pdf.locked.title": "🔒 दिन 3 – 7 Premium के साथ उपलब्ध हैं",
    "pdf.locked.sub": "रेसिपी, सामग्री और खरीदारी सूची के साथ पूरा 7-दिन का प्लान अनलॉक करें।",
    "pdf.locked.cta": "पूर्ण पहुँच प्राप्त करें →",
    "pdf.tempUnavailable": "इस भाषा के लिए PDF निर्यात अस्थायी रूप से अनुपलब्ध है। बाकी सब सामान्य रूप से काम कर रहा है।",
    placeholderL: "उदा: आलू, प्याज",
    placeholderD: "उदा: चिकन, चावल",
    "banner.unlock": "क्या आपने भुगतान कर दिया है? असीमित PDF डाउनलोड अनलॉक करें",
    "input.email": "अपना भुगतान ईमेल दर्ज करें",
    "access.granted": "✅ पहुँच प्राप्त! आप अनलिमिटेड PDF डाउनलोड कर सकते हैं:",
    "feedback.header": "त्वरित प्रतिक्रिया भेजें",
    "feedback.placeholder": "यहां सुझाव, समस्या या प्रतिक्रिया लिखें...",
    "feedback.button": "प्रतिक्रिया भेजें",
    "feedback.thanks": "आपकी प्रतिक्रिया के लिए धन्यवाद! ईमेल खुल जाएगा।",
    "feedback.error": "कृपया संदेश लिखें।",
    "feedback.note": "आपकी प्रतिक्रिया सीधे डेवलपर को जाएगी।",
    "msg.valid": "✅ पहुँच प्राप्त! आप अनलिमिटेड PDF डाउनलोड कर सकते हैं:",
    "costEstimate": "अनुमानित लागत",
    "perWeek": "सप्ताह · 2 लोग",
    "estWeeklyCost": "साप्ताहिक अनुमानित लागत",
    "hero.badge": "मुफ्त · 14 भाषाएं · 100+ रेसिपी",
    "hero.sub": "अपने सप्ताह की योजना बनाएं, शॉपिंग लिस्ट अपने आप बनाएं और PDF डाउनलोड करें — मुफ्त, किसी भी भाषा में।",
    "hero.social": "दुनिया भर में 14 भाषाओं में उपयोग 🌍",
    "hero.cta": "🥗 मेरी मुफ्त योजना बनाएं",
    "mealsFound": "भोजन मिले",
    "how.title": "यह कैसे काम करता है",
    "how.step1.title": "भोजन जोड़ें या बनाएं",
    "how.step1.desc": "अपना भोजन टाइप करें या पूरे सप्ताह को स्वचालित भरने के लिए «बनाएं» क्लिक करें",
    "how.step2.title": "खरीदारी सूची — स्वचालित",
    "how.step2.desc": "सामग्री आपके प्लान से स्वचालित रूप से व्यवस्थित की जाती है",
    "how.step3.title": "PDF के रूप में डाउनलोड करें",
    "how.step3.desc": "एक सुंदर प्रिंट योग्य प्लान पाएं, दिन में एक बार मुफ्त"
  },
  tr: {
  title: "Haftalık Yemek Planlayıcı",
  header: "Yemek Planlayıcı & Alışveriş Listesi",
  weekdays: ['Pazartesi','Salı','Çarşamba','Perşembe','Cuma','Cumartesi','Pazar'],
  msg: {
    checking: "Kontrol ediliyor...",
    empty: "Lütfen e-postanızı girin!",
    server_error: "Sunucu hatası. Lütfen tekrar deneyin.",
    invalid: "Bu e-posta için geçerli erişim bulunamadı.",
    not_found: "Erişim bulunamadı. Lütfen önce ödeme yapın veya e-postayı kontrol edin.",
    valid: "✅ Erişim onaylandı! Sınırsız PDF indirebilirsiniz:"
  },
  "btn.generate": "PDF Oluştur",
  "export.title": "PDF İndir",
  "export.sub": "Planınız, tarifler ve alışveriş listesi",
  "btn.pay": "Premium – €3/ay",
  "btn.autoMenu": "Rastgele menü oluştur",
  "budget.toggle": "Bütçe menüsü (kriz modu)",
  "btn.verify": "E-postayı Doğrula",
  "btn.switchAccount": "Hesabı değiştir",
  "btn.download": "PDF İndir",
  "col.day": "Gün",
  "col.lunch": "Öğle yemeği (malzemeler)",
  "col.dinner": "Akşam yemeği (malzemeler)",
  "pw.lunch": "Öğle yemeği",
  "pw.dinner": "Akşam yemeği",
  "pw.ovDays": "Planlanan günler",
  "pw.ovTime": "Ortalama süre",
  "pw.ovCost": "Tahmini maliyet",
  "pw.ovCuisines": "Mutfaklar",
  "pw.reroll": "Başka tarif",
  "pw.remove": "Öğünü kaldır",
  "pw.undo": "Geri al",
  "pw.clearPlan": "Planı temizle",
  "pw.cleared": "Plan temizlendi",
  "pw.regenerated": "Yeni plan oluşturuldu",
  "pw.fromCart": "tarif seçiminden eklendi",
  "pw.removed": "kaldırıldı",
  "pw.noMore": "Bu filtredeki tüm tarifleri kullandınız",
  "pw.addLunch": "Öğle yemeği ekle",
  "pw.addDinner": "Akşam yemeği ekle",
  "pw.searchHint": "tarif veya malzemeye göre ara",
  "pw.searchPh": "Tarif veya malzeme ara…",
  "pw.noResults": "Sonuç yok. Bir malzeme dene: tavuk, pirinç, yumurta…",
  "pw.contains": "{ing} içerir",
  "pw.recommended": "Planına uygun",
  "pw.quickCheap": "Hızlı ve ucuz",
  "pw.favs": "Favorileriniz",
  "pw.allRecipes": "Tüm tarifler",
  "pw.alreadyIn": "planda zaten var · {day}",
  "col.origin": "Ülke",
  "col.ingredients": "Malzemeler",
  "howIsMade": "Nasıl yapılır:",
  shoppingList: "Alışveriş Listesi",
  "shopping.teaser.desc": "Otomatik olarak kategorilere göre düzenlenir, PDF'te alışverişe hazır.",
  "shopping.empty": "Liste oluşturmak için yemek ekleyin",
  maxed: "Ücretsiz PDF limitine ulaştınız.<br>Sınırsız indirme için ödeme yapın!",
  "pdf.free.label": "Ücretsiz önizleme — 7 günden 2'si",
  "mode.meal": "Bir öğün", "mode.day": "Bir gün", "mode.week": "Bir hafta",
  "mode.heading.meal": "Öğün seç", "mode.heading.day": "Bugünün planı", "mode.heading.week": "Haftalık plan",
  "mode.btn.meal": "✨ Beni şaşırt!", "mode.btn.day": "Günü oluştur", "mode.btn.week": "Haftayı oluştur",
  "mode.col.meal": "Önerilen yemek", "today": "Bugün",
  "pdf.locked.title": "🔒 3 – 7. günler Premium ile kullanılabilir",
  "pdf.locked.sub": "Tarif, malzeme ve alışveriş listesiyle 7 günlük tam planı açın.",
  "pdf.locked.cta": "Tam erişim al →",
  "pdf.tempUnavailable": "Bu dil için PDF dışa aktarma geçici olarak kullanılamıyor. Diğer her şey normal şekilde çalışıyor.",
  placeholderL: "örn: patates, soğan",
  placeholderD: "örn: tavuk, pilav",
  "banner.unlock": "Daha önce ödeme yaptınız mı? Sınırsız PDF indirmeyi açın",
  "input.email": "Ödeme e-postanızı girin",
  "access.granted": "✅ Erişim onaylandı! Sınırsız PDF indirebilirsiniz:",
  "feedback.header": "Hızlı geri bildirim gönder",
  "feedback.placeholder": "Önerilerinizi, sorunları veya görüşleri buraya yazın...",
  "feedback.button": "Gönder",
  "feedback.thanks": "Teşekkürler! E-posta gönderimi için açılacaktır.",
  "feedback.error": "Lütfen bir mesaj yazın.",
  "feedback.note": "Geri bildiriminiz doğrudan geliştiriciye ulaşır.",
  "msg.valid": "✅ Erişim onaylandı! Sınırsız PDF indirebilirsiniz:",
    "costEstimate": "Tahmini maliyet",
    "perWeek": "hafta · 2 kişi",
    "estWeeklyCost": "Tahmini haftalık maliyet",
  "hero.badge": "Ücretsiz · 14 dil · 100+ tarif",
  "hero.sub": "Haftanızı planlayın, alışveriş listelerini otomatik oluşturun ve PDF indirin — ücretsiz, her dilde.",
  "hero.social": "Dünya genelinde 14 dilde kullanılıyor 🌍",
  "hero.cta": "🥗 Ücretsiz Planımı Oluştur",
  "mealsFound": "yemek bulundu",
  "how.title": "Nasıl çalışır",
  "how.step1.title": "Öğünleri ekleyin veya oluşturun",
  "how.step1.desc": "Öğünlerinizi yazın veya haftayı otomatik doldurmak için «Oluştur»'a tıklayın",
  "how.step2.title": "Alışveriş listesi — otomatik",
  "how.step2.desc": "Malzemeler planınızdan otomatik olarak düzenlenir",
  "how.step3.title": "PDF olarak indirin",
  "how.step3.desc": "Günde bir kez ücretsiz güzel bir yazdırılabilir plan alın"
},
it: {
  title: "Pianificatore Settimanale dei Pasti",
  header: "Pianificatore dei Pasti & Lista della Spesa",
  weekdays: ['Lunedì','Martedì','Mercoledì','Giovedì','Venerdì','Sabato','Domenica'],
  msg: {
    checking: "Verifica in corso...",
    empty: "Inserisci la tua email!",
    server_error: "Errore del server. Riprova.",
    invalid: "Nessun accesso valido trovato per questa email.",
    not_found: "Accesso non trovato. Effettua il pagamento o controlla l’email.",
    valid: "✅ Accesso confermato! Puoi scaricare PDF illimitati:"
  },
  "btn.generate": "Genera PDF",
  "export.title": "Scarica PDF",
  "export.sub": "Il tuo piano, ricette e lista della spesa",
  "btn.pay": "Premium – €3/mese",
  "btn.autoMenu": "Genera menu casuale",
  "budget.toggle": "Menu economico (modalità crisi)",
  "btn.verify": "Verifica Email",
  "btn.switchAccount": "Cambia account",
  "btn.download": "Scarica PDF",
  "col.day": "Giorno",
  "col.lunch": "Pranzo (ingredienti)",
  "col.dinner": "Cena (ingredienti)",
  "pw.lunch": "Pranzo",
  "pw.dinner": "Cena",
  "pw.ovDays": "Giorni pianificati",
  "pw.ovTime": "Tempo medio",
  "pw.ovCost": "Costo stimato",
  "pw.ovCuisines": "Cucine",
  "pw.reroll": "Un'altra ricetta",
  "pw.remove": "Rimuovi il pasto",
  "pw.undo": "Annulla",
  "pw.clearPlan": "Svuota il piano",
  "pw.cleared": "Piano svuotato",
  "pw.regenerated": "Nuovo piano generato",
  "pw.fromCart": "aggiunte dalla tua selezione",
  "pw.removed": "rimosso",
  "pw.noMore": "Hai usato tutte le ricette di questo filtro",
  "pw.addLunch": "Aggiungi il pranzo",
  "pw.addDinner": "Aggiungi la cena",
  "pw.searchHint": "cerca per ricetta o ingrediente",
  "pw.searchPh": "Cerca ricetta o ingrediente…",
  "pw.noResults": "Nessun risultato. Prova un ingrediente: pollo, riso, uova…",
  "pw.contains": "contiene {ing}",
  "pw.recommended": "Adatte al tuo piano",
  "pw.quickCheap": "Veloci ed economiche",
  "pw.favs": "I tuoi preferiti",
  "pw.allRecipes": "Tutte le ricette",
  "pw.alreadyIn": "già nel piano · {day}",
  "col.origin": "Paese",
  "col.ingredients": "Ingredienti",
  "howIsMade": "Preparazione:",
  shoppingList: "Lista della Spesa",
  "shopping.teaser.desc": "Organizzata automaticamente per categoria e pronta per la spesa nel PDF.",
  "shopping.empty": "Aggiungi pasti per generare la tua lista",
  maxed: "Hai raggiunto il limite di PDF gratuiti.<br>Paga per sbloccare download illimitati!",
  "pdf.free.label": "Anteprima gratuita — 2 giorni su 7",
  "mode.meal": "Un pasto", "mode.day": "Un giorno", "mode.week": "Una settimana",
  "mode.heading.meal": "Scegli un pasto", "mode.heading.day": "Piano di oggi", "mode.heading.week": "Piano settimanale",
  "mode.btn.meal": "✨ Sorprendimi!", "mode.btn.day": "Genera la giornata", "mode.btn.week": "Genera la settimana",
  "mode.col.meal": "Pasto suggerito", "today": "Oggi",
  "pdf.locked.title": "🔒 I giorni 3 – 7 sono disponibili con Premium",
  "pdf.locked.sub": "Sblocca il piano completo di 7 giorni con ricette, ingredienti e lista della spesa.",
  "pdf.locked.cta": "Ottieni accesso completo →",
  "pdf.tempUnavailable": "L'esportazione PDF è temporaneamente non disponibile per questa lingua. Tutto il resto continua a funzionare.",
  placeholderL: "es: patate, cipolla",
  placeholderD: "es: pollo, riso",
  "banner.unlock": "Hai già pagato? Sblocca i download PDF illimitati",
  "input.email": "Inserisci l’email di pagamento",
  "access.granted": "✅ Accesso confermato! Puoi scaricare PDF illimitati:",
  "feedback.header": "Invia un feedback rapido",
  "feedback.placeholder": "Scrivi qui suggerimenti, problemi o opinioni...",
  "feedback.button": "Invia",
  "feedback.thanks": "Grazie! Si aprirà l’email per l’invio.",
  "feedback.error": "Scrivi un messaggio.",
  "feedback.note": "Il tuo feedback arriva direttamente allo sviluppatore.",
  "msg.valid": "✅ Accesso confermato! Puoi scaricare PDF illimitati:",
    "costEstimate": "Costo stimato",
    "perWeek": "settimana · 2 persone",
    "estWeeklyCost": "Costo settimanale stimato",
  "hero.badge": "Gratuito · 14 lingue · 100+ ricette",
  "hero.sub": "Pianifica la tua settimana, genera automaticamente liste della spesa e scarica PDF — gratis, in qualsiasi lingua.",
  "hero.social": "Usato in 14 lingue in tutto il mondo 🌍",
  "hero.cta": "🥗 Genera Il Mio Piano Gratuito",
  "mealsFound": "pasti trovati",
  "how.title": "Come funziona",
  "how.step1.title": "Aggiungi o genera i pasti",
  "how.step1.desc": "Scrivi i tuoi pasti o clicca «Genera» per compilare automaticamente la settimana",
  "how.step2.title": "Lista della spesa — automatica",
  "how.step2.desc": "Gli ingredienti vengono organizzati automaticamente dal tuo piano",
  "how.step3.title": "Scarica come PDF",
  "how.step3.desc": "Ottieni un bel piano stampabile, gratuito una volta al giorno"
},
ko: {
  title: "주간 식단 플래너",
  header: "식단 플래너 & 장보기 리스트",
  weekdays: ['월요일','화요일','수요일','목요일','금요일','토요일','일요일'],
  msg: {
    checking: "확인 중...",
    empty: "이메일을 입력하세요!",
    server_error: "서버 오류입니다. 다시 시도해주세요.",
    invalid: "해당 이메일로 유효한 접근 권한을 찾을 수 없습니다.",
    not_found: "접근 권한이 없습니다. 먼저 결제하거나 이메일을 확인하세요.",
    valid: "✅ 접근 허가됨! 무제한 PDF 다운로드가 가능합니다:"
  },
  "btn.generate": "PDF 생성",
  "export.title": "PDF 다운로드",
  "export.sub": "플랜, 레시피 & 쇼핑 목록",
  "btn.pay": "프리미엄 – €3/월",
  "btn.autoMenu": "랜덤 메뉴 생성",
  "budget.toggle": "절약 메뉴(위기 모드)",
  "btn.verify": "이메일 확인",
  "btn.switchAccount": "계정 전환",
  "btn.download": "PDF 다운로드",
  "col.day": "요일",
  "col.lunch": "점심 (재료)",
  "col.dinner": "저녁 (재료)",
  "pw.lunch": "점심",
  "pw.dinner": "저녁",
  "pw.ovDays": "계획된 날",
  "pw.ovTime": "평균 시간",
  "pw.ovCost": "예상 비용",
  "pw.ovCuisines": "요리 종류",
  "pw.reroll": "다른 레시피",
  "pw.remove": "식사 제거",
  "pw.undo": "실행 취소",
  "pw.clearPlan": "플랜 비우기",
  "pw.cleared": "플랜을 비웠습니다",
  "pw.regenerated": "새 플랜이 생성되었습니다",
  "pw.fromCart": "개 레시피를 담기에서 추가",
  "pw.removed": "제거됨",
  "pw.noMore": "이 필터의 레시피를 모두 사용했습니다",
  "pw.addLunch": "점심 추가",
  "pw.addDinner": "저녁 추가",
  "pw.searchHint": "레시피나 재료로 검색",
  "pw.searchPh": "레시피 또는 재료 검색…",
  "pw.noResults": "결과가 없습니다. 재료로 검색해 보세요: 닭고기, 쌀, 달걀…",
  "pw.contains": "{ing} 포함",
  "pw.recommended": "내 플랜에 어울리는",
  "pw.quickCheap": "빠르고 저렴한",
  "pw.favs": "즐겨찾기",
  "pw.allRecipes": "모든 레시피",
  "pw.alreadyIn": "이미 플랜에 있음 · {day}",
  "col.origin": "국가",
  "col.ingredients": "재료",
  "howIsMade": "만드는 방법:",
  shoppingList: "장보기 리스트",
  "shopping.teaser.desc": "카테고리별로 자동 정리되어 PDF에서 바로 장보기 가능합니다.",
  "shopping.empty": "식사를 추가해 목록을 생성하세요",
  maxed: "무료 PDF 한도에 도달했습니다.<br>무제한 다운로드를 위해 결제하세요!",
  "pdf.free.label": "무료 미리보기 — 7일 중 2일",
  "mode.meal": "한 끼", "mode.day": "하루", "mode.week": "일주일",
  "mode.heading.meal": "한 끼 선택", "mode.heading.day": "오늘의 플랜", "mode.heading.week": "주간 플랜",
  "mode.btn.meal": "✨ 깜짝 추천!", "mode.btn.day": "오늘 생성", "mode.btn.week": "한 주 생성",
  "mode.col.meal": "추천 요리", "today": "오늘",
  "pdf.locked.title": "🔒 3 – 7일은 Premium으로 이용 가능",
  "pdf.locked.sub": "레시피, 재료, 장보기 목록이 포함된 7일 전체 플랜을 잠금 해제하세요.",
  "pdf.locked.cta": "전체 이용권 얻기 →",
  "pdf.tempUnavailable": "이 언어의 PDF 내보내기는 일시적으로 사용할 수 없습니다. 다른 기능은 정상적으로 작동합니다.",
  placeholderL: "예: 감자, 양파",
  placeholderD: "예: 치킨, 밥",
  "banner.unlock": "이미 결제하셨나요? PDF 무제한 다운로드 활성화",
  "input.email": "결제 이메일 입력",
  "access.granted": "✅ 접근 허가됨! 무제한 PDF 다운로드 가능:",
  "feedback.header": "빠른 피드백 보내기",
  "feedback.placeholder": "여기에 의견, 문제 또는 제안을 적어주세요...",
  "feedback.button": "보내기",
  "feedback.thanks": "감사합니다! 이메일이 열립니다.",
  "feedback.error": "메시지를 입력하세요.",
  "feedback.note": "피드백은 개발자에게 직접 전달됩니다.",
  "msg.valid": "✅ 접근 허가됨! 무제한 PDF 다운로드 가능:",
    "costEstimate": "예상 비용",
    "perWeek": "주 · 2인",
    "estWeeklyCost": "주간 예상 비용",
  "hero.badge": "무료 · 14개 언어 · 100개+ 레시피",
  "hero.sub": "한 주를 계획하고, 쇼핑 목록을 자동 생성하고, PDF를 다운로드하세요 — 무료, 모든 언어로.",
  "hero.social": "전 세계 14개 언어로 사용 🌍",
  "hero.cta": "🥗 무료 식단 계획 만들기",
  "mealsFound": "식사 찾음",
  "how.title": "이용 방법",
  "how.step1.title": "식사 추가 또는 자동 생성",
  "how.step1.desc": "식사를 입력하거나 «생성»을 클릭해 한 주를 자동으로 채우세요",
  "how.step2.title": "장보기 목록 — 자동 생성",
  "how.step2.desc": "재료가 계획에서 자동으로 정리됩니다",
  "how.step3.title": "PDF로 다운로드",
  "how.step3.desc": "아름다운 인쇄용 플랜을 하루 1회 무료로 받으세요"
}
};

  // 4. Mesaj impact pentru PDF (personalizabil pe limbă)
  const pdfMessages = {
    ro: "Ai grijă de sănătatea ta și a familiei tale! Alege cumpătat, gătește cu drag și bucură-te de fiecare masă!",
    en: "Take care of your health and your family! Cook with love and enjoy every meal!",
    es: "¡Cuida tu salud y la de tu familia! ¡Cocina con amor y disfruta cada comida!",
    fr: "Prenez soin de votre santé et de votre famille ! Cuisinez avec amour et profitez de chaque repas !",
    ru: "Берегите здоровье своё и своей семьи! Готовьте с любовью и наслаждайтесь каждым приёмом пищи!",
    zh: "关爱自己和家人的健康！用心烹饪，享受每一餐！",
    ja: "あなたと家族の健康を大切に！愛情を込めて料理し、毎日の食事を楽しんでください！",
    pt: "Cuide da sua saúde e da sua família! Cozinhe com amor e aproveite cada refeição!",
    de: "Achte auf deine Gesundheit und die deiner Familie! Koche mit Liebe und genieße jede Mahlzeit!",
    ar: "اعتنِ بصحتك وصحة عائلتك! اطبخ بحب واستمتع بكل وجبة!",
    hi: "अपनी और अपने परिवार की सेहत का ख्याल रखें! प्यार से पकाएं और हर भोजन का आनंद लें!",
    tr: "Sağlığınıza ve ailenize önem verin! Sevgiyle pişirin ve her öğünün tadını çıkarın!",
    it: "Abbi cura della tua salute e della tua famiglia! Cucina con amore e goditi ogni pasto!",
    ko: "당신과 가족의 건강을 소중히 하세요! 사랑을 담아 요리하고 매 순간을 즐기세요!"
  };

  // 5. Populează selectul de limbă
  const langNames = {
    ro: "Română", en: "English", es: "Español", fr: "Français",
    ru: "Русский", zh: "中文", ja: "日本語", pt: "Português",
    de: "Deutsch", ar: "العربية", hi: "हिन्दी",tr: "Türkçe",
    it: "Italiano", ko: "한국어"
  };
  const seoParagraphs = {
  ro: `
  <p>
    <strong>Aplică organizarea meselor zilnice cu planificatorul nostru săptămânal!</strong> Descoperă rețete tradiționale și moderne din toate colțurile lumii – de la preparate românești și italiene, până la mâncăruri asiatice și deserturi americane. Planifică meniul fiecărei zile, adaugă rețete și ingrediente, iar la final obții automat lista de cumpărături pentru întreaga săptămână, gata de tipărit sau salvat PDF.
  </p>
  <p>
    Instrumentul online este ideal pentru familii ocupate, persoane care țin dietă, sau oricine dorește să economisească timp și să reducă risipa alimentară. Gătești mai eficient, faci cumpărături mai rapid și controlezi mai ușor bugetul casei.
  </p>
  <p>
    Folosește plannerul gratuit, în limba română – nu necesită cont, fără reclame, direct din browser!
  </p>
`,
  ar: `
    <p>
      <strong>مخطط وجبات أسبوعي عبر الإنترنت</strong> – أنشئ خطة وجباتك للأسبوع بالكامل، ووفر الوقت والمال، واستمتع بوجبات صحية مع عائلتك. تطبيقنا المجاني يتيح لك تسجيل وجباتك اليومية، وإنشاء قائمة تسوق تلقائياً، وتحميل كل ذلك بصيغة PDF.
    </p>
    <p>
      سواء كنت ترغب في تنظيم وجبات الأطفال، أو اتباع نظام غذائي متوازن، أو فقط ترغب في إدارة ميزانيتك بشكل أفضل، هذا المخطط يساعدك على الالتزام بالخطة وتجنب هدر الطعام. كل شيء سريع وسهل الاستخدام عبر الإنترنت، ويمكنك استخدام التطبيق بأي لغة تريدها!
    </p>
    <p>
      جرب مخطط الوجبات الأسبوعي المجاني – بسيط، مفيد، ولا حاجة لحساب!
    </p>
  `,
  de: `
    <p>
      <strong>Wöchentlicher Essensplaner online</strong> – Erstelle deinen Menüplan für die ganze Woche, spare Zeit und Geld und genieße gesunde Mahlzeiten mit der Familie. Unsere kostenlose App hilft dir dabei, deine täglichen Gerichte festzuhalten, automatisch eine Einkaufsliste zu generieren und alles als PDF herunterzuladen.
    </p>
    <p>
      Egal ob du die Mahlzeiten für Kinder organisieren möchtest, einer ausgewogenen Ernährung folgen willst oder einfach effizienter mit deinem Budget umgehen möchtest – dieser Planer hilft dir, am Plan zu bleiben und Lebensmittelverschwendung zu vermeiden. Alles ist schnell, online und du kannst die App in jeder gewünschten Sprache nutzen!
    </p>
    <p>
      Probiere den kostenlosen wöchentlichen Essensplaner aus – einfach, praktisch und ohne Anmeldung!
    </p>
  `,
  en: `
    <p>
      <strong>Weekly Meal Planner Online</strong> – Plan your entire week's menu, save time and money, and enjoy healthy meals with your family. Our free app lets you organize daily recipes, automatically generate a shopping list, and download everything as a PDF.
    </p>
    <p>
      Whether you want to meal prep for your kids, follow a balanced diet, or just be more efficient with your budget, this planner helps you stay organized and avoid food waste. Fast, online, and available in any language you choose!
    </p>
    <p>
      Try our free weekly meal planner – simple, useful, and no registration needed!
    </p>
  `,
  es: `
    <p>
      <strong>Planificador semanal de comidas online</strong> – Organiza el menú de toda la semana, ahorra tiempo y dinero, y disfruta de comidas saludables en familia. Nuestra aplicación gratuita te permite anotar lo que cocinas cada día, generar automáticamente la lista de la compra y descargar todo en PDF.
    </p>
    <p>
      Ya sea que quieras planificar comidas para los niños, seguir una dieta equilibrada o simplemente gestionar mejor tu presupuesto, este planificador te ayuda a cumplir tu plan y evitar el desperdicio de alimentos. ¡Todo es rápido, online y puedes usar la app en cualquier idioma que desees!
    </p>
    <p>
      ¡Prueba el planificador semanal de comidas gratis – sencillo, útil y sin registro!
    </p>
  `,
  fr: `
    <p>
      <strong>Planificateur de repas hebdomadaire en ligne</strong> – Créez votre menu pour toute la semaine, gagnez du temps et de l’argent, et profitez de repas sains en famille. Notre application gratuite vous permet de noter vos plats chaque jour, de générer automatiquement la liste de courses et de tout télécharger au format PDF.
    </p>
    <p>
      Que vous souhaitiez organiser les repas pour les enfants, suivre un régime équilibré ou simplement gérer votre budget plus efficacement, ce planificateur vous aide à respecter votre planning et à éviter le gaspillage alimentaire. Tout est rapide, en ligne, et vous pouvez utiliser l’application dans la langue de votre choix !
    </p>
    <p>
      Essayez le planificateur de repas hebdomadaire gratuit – simple, utile et sans inscription !
    </p>
  `,
  hi: `
    <p>
      <strong>साप्ताहिक भोजन योजना ऑनलाइन</strong> – पूरे सप्ताह का मेनू बनाएं, समय और पैसे बचाएं और परिवार के साथ सेहतमंद भोजन का आनंद लें। हमारी फ्री ऐप से आप रोज़ाना का भोजन प्लान कर सकते हैं, शॉपिंग लिस्ट अपने आप बना सकते हैं और सब कुछ PDF में डाउनलोड कर सकते हैं।
    </p>
    <p>
      चाहे आप बच्चों के लिए भोजन प्लान करना चाहते हों, संतुलित डाइट फॉलो करना चाहते हों या बजट को बेहतर बनाना चाहते हों – यह प्लानर आपको संगठित रहने और भोजन की बर्बादी रोकने में मदद करेगा। सब कुछ तेज़, ऑनलाइन और किसी भी भाषा में उपलब्ध!
    </p>
    <p>
      मुफ्त साप्ताहिक भोजन योजना आज़माएँ – आसान, उपयोगी और बिना रजिस्ट्रेशन!
    </p>
  `,
  ja: `
  <p>
    <strong>週間ミールプランナー（日本版）</strong> – 忙しい毎日でも、家族の健康を守りながら、手軽に1週間分の食事を計画できます。無料アプリで日々の献立や買い物リストを簡単に管理、PDFでダウンロードも可能です。
  </p>
  <p>
    弁当作りや栄養バランス、予算管理など、日本のご家庭のニーズにぴったり対応。和食から洋食まで幅広いメニューに対応し、食品ロス削減にも貢献します。
  </p>
  <p>
    今すぐ無料で使ってみましょう！レシピ例や家族のためのヒントも掲載中。日本語で安心してご利用いただけます。
  </p>
`,
  pt: `
    <p>
      <strong>Planejador semanal de refeições online</strong> – Monte o cardápio da semana inteira, economize tempo e dinheiro, e aproveite refeições saudáveis com a família. Nosso aplicativo gratuito permite anotar o que você vai cozinhar, gerar a lista de compras automaticamente e baixar tudo em PDF.
    </p>
    <p>
      Seja para organizar as refeições das crianças, seguir uma dieta equilibrada ou controlar melhor o orçamento, este planejador ajuda você a seguir o plano e evitar desperdício de alimentos. Tudo é rápido, online e disponível em qualquer idioma!
    </p>
    <p>
      Experimente o planejador semanal de refeições grátis – simples, útil e sem cadastro!
    </p>
  `,
  ru: `
    <p>
      <strong>Еженедельный планировщик питания онлайн</strong> – Составляйте меню на всю неделю, экономьте время и деньги, наслаждайтесь полезными семейными обедами и ужинами. Наше бесплатное приложение позволит вам записывать блюда на каждый день, автоматически создавать список покупок и скачивать всё в PDF.
    </p>
    <p>
      Хотите планировать питание для детей, придерживаться сбалансированной диеты или просто эффективно расходовать бюджет – этот планировщик поможет вам следовать плану и избежать пищевых отходов. Всё быстро, онлайн, и вы можете выбрать любой язык!
    </p>
    <p>
      Попробуйте бесплатный еженедельный планировщик питания – просто, удобно и без регистрации!
    </p>
  `,
  zh: `
    <p>
      <strong>每周膳食计划在线工具</strong> – 制定一整周的菜单，节省时间和金钱，与家人一起享受健康美味的餐食。我们的免费应用让你记录每天的食谱，自动生成购物清单，并支持一键下载PDF。
    </p>
    <p>
      无论是为孩子安排饮食、坚持均衡营养，还是提升家庭预算效率，这款计划工具都能帮你更好地坚持计划，减少食物浪费。一切都很快捷、在线，并可支持多种语言！
    </p>
    <p>
      立即体验免费的每周膳食计划工具 – 简单、实用、无需注册！
    </p>
  `,
  tr: `
  <p>
    <strong>Haftalık yemek planlayıcımız ile haftanın tüm öğünlerini düzenleyin!</strong> 
    Dünya mutfaklarından sağlıklı ve pratik tarifleri keşfedin. Aileniz için haftalık menü oluşturun, malzemeleri kaydedin ve otomatik alışveriş listesi oluşturun.
  </p>
  <p>
    Yoğun aileler, diyet yapanlar veya bütçesini daha iyi yönetmek isteyen herkes için idealdir. 
    Daha az israf, daha fazla tasarruf ve daha fazla zaman kazanın.
  </p>
  <p>
    Ücretsiz ve çevrimiçi planlayıcımızı Türkçe kullanabilir, PDF olarak kaydedebilirsiniz!
  </p>
`,
it: `
  <p>
    <strong>Organizza i pasti della tua settimana con il nostro pianificatore!</strong> 
    Scopri ricette semplici e gustose provenienti da tutto il mondo. Pianifica ogni giorno, gestisci gli ingredienti e genera automaticamente la lista della spesa.
  </p>
  <p>
    Perfetto per famiglie impegnate, chi segue una dieta o chi vuole risparmiare tempo e denaro evitando sprechi alimentari.
  </p>
  <p>
    Usa il pianificatore gratuitamente, completamente in italiano, senza registrazione!
  </p>
`,
ko: `
  <p>
    <strong>주간 식단 플래너로 일주일의 식단을 쉽게 관리하세요!</strong>
    전 세계 다양한 요리 레시피를 확인하고 하루 식단을 미리 계획해 보세요. 재료를 정리하면 자동으로 장보기 리스트가 생성됩니다.
  </p>
  <p>
    바쁜 가족, 다이어트 중인 사람들, 식비를 절약하고 싶은 모든 사람에게 완벽한 도구입니다.
  </p>
  <p>
    회원가입 없이 무료로 사용할 수 있으며, 한국어 PDF 다운로드도 가능합니다!
  </p>
`
};
// ===== Mesaje motivaționale (random, multi-limbă)
const MOTIV = {
  ro: [
    "Felicitări! Ai reușit să îți planifici toate mesele săptămânii! 💪 Consistența ta este cheia unui stil de viață sănătos. Continuă așa!",
    "Bună treabă! Un plan clar azi înseamnă mai puțin stres și mai mult timp pentru tine mâine.",
    "Pas cu pas, obiceiurile bune se construiesc. Planul tău de azi contează!",
    "Planificarea este începutul succesului. Azi ai făcut un pas mare spre obiectivele tale!",
    "Fiecare masă planificată este un pas spre o viață mai echilibrată.",
    "Ai grijă de tine, un plan bun înseamnă sănătate pe termen lung.",
    "Disciplina de azi este libertatea de mâine. Bravo!",
    "Ai transformat intențiile în acțiuni. Continuă să faci asta!",
    "Planificarea ta arată determinare și grijă pentru tine și cei dragi.",
    "Un plan bun este ca o hartă — te duce exact unde vrei să ajungi."
  ],
  en: [
    "Great job! You planned all your meals for the week! 💪 Consistency is your superpower—keep going!",
    "Nice work! A clear plan today means less stress and more time tomorrow.",
    "Step by step, good habits stick. Today’s plan matters!",
    "Planning is the first step to success. You’ve nailed it today!",
    "Every planned meal is a step toward a balanced life.",
    "Take care of yourself—good planning means long-term health.",
    "Discipline today is freedom tomorrow. Well done!",
    "You turned intentions into actions. Keep it up!",
    "Your planning shows determination and care for yourself and loved ones.",
    "A good plan is like a map—it gets you exactly where you want to go."
  ],
  es: [
    "¡Felicidades! ¡Has planificado todas tus comidas de la semana! 💪 La constancia es la clave. ¡Sigue así!",
    "¡Buen trabajo! Un plan claro hoy significa menos estrés mañana.",
    "Paso a paso, los buenos hábitos se construyen. ¡Tu plan cuenta!",
    "Planificar es el primer paso hacia el éxito. ¡Lo lograste hoy!",
    "Cada comida planificada es un paso hacia una vida equilibrada.",
    "Cuídate: una buena planificación significa salud a largo plazo.",
    "La disciplina de hoy es la libertad de mañana. ¡Bien hecho!",
    "Has convertido las intenciones en acciones. ¡Sigue así!",
    "Tu planificación muestra determinación y cuidado por ti y tus seres queridos.",
    "Un buen plan es como un mapa: te lleva justo donde quieres llegar."
  ],
  fr: [
    "Bravo ! Tu as planifié tous tes repas de la semaine ! 💪 La constance fait la différence. Continue !",
    "Super travail ! Un plan clair aujourd’hui = moins de stress demain.",
    "Pas à pas, les bonnes habitudes s’installent. Ton plan compte !",
    "Planifier est la première étape vers le succès. Tu as réussi aujourd’hui !",
    "Chaque repas planifié est un pas vers une vie équilibrée.",
    "Prends soin de toi : une bonne planification signifie santé sur le long terme.",
    "La discipline d’aujourd’hui est la liberté de demain. Bravo !",
    "Tu as transformé tes intentions en actions. Continue comme ça !",
    "Ta planification montre ta détermination et ton attention pour toi et tes proches.",
    "Un bon plan, c’est comme une carte : il t’emmène là où tu veux aller."
  ],
  de: [
    "Stark! Deine Wochenmahlzeiten stehen! 💪 Dranbleiben ist der Schlüssel. Weiter so!",
    "Gute Arbeit! Ein klarer Plan heute heißt weniger Stress morgen.",
    "Schritt für Schritt werden gute Gewohnheiten gefestigt. Dein Plan zählt!",
    "Planung ist der erste Schritt zum Erfolg. Heute hast du es geschafft!",
    "Jede geplante Mahlzeit ist ein Schritt zu einem ausgewogenen Leben.",
    "Kümmere dich um dich: Gute Planung bedeutet langfristige Gesundheit.",
    "Disziplin heute ist Freiheit morgen. Gut gemacht!",
    "Du hast Absichten in Taten umgesetzt. Mach weiter so!",
    "Deine Planung zeigt Entschlossenheit und Fürsorge für dich und deine Lieben.",
    "Ein guter Plan ist wie eine Landkarte – er bringt dich genau ans Ziel."
  ],
  pt: [
    "Parabéns! Planeaste as refeições da semana! 💪 A consistência é a chave. Continua!",
    "Bom trabalho! Um plano claro hoje = menos stress amanhã.",
    "Passo a passo, os bons hábitos ficam. O teu plano conta!",
    "Planear é o primeiro passo para o sucesso. Hoje conseguiste!",
    "Cada refeição planeada é um passo para uma vida equilibrada.",
    "Cuida de ti: um bom planeamento significa saúde a longo prazo.",
    "A disciplina de hoje é a liberdade de amanhã. Muito bem!",
    "Transformaste intenções em ações. Continua assim!",
    "O teu planeamento mostra determinação e cuidado contigo e com os teus.",
    "Um bom plano é como um mapa – leva-te exatamente onde queres chegar."
  ],
  ru: [
    "Отлично! Ты распланировал(а) питание на неделю! 💪 Постоянство — твоя сила. Продолжай!",
    "Хорошая работа! Чёткий план сегодня — меньше стресса завтра.",
    "Шаг за шагом прививаются полезные привычки. Твой план важен!",
    "Планирование — первый шаг к успеху. Сегодня ты это сделал(а)!",
    "Каждый запланированный приём пищи — шаг к сбалансированной жизни.",
    "Береги себя: хорошее планирование — это здоровье на долгие годы.",
    "Дисциплина сегодня — свобода завтра. Молодец!",
    "Ты превратил(а) намерения в действия. Продолжай!",
    "Твоё планирование показывает решимость и заботу о себе и близких.",
    "Хороший план — как карта: ведёт тебя прямо к цели."
  ],
  ar: [
    "أحسنت! لقد خططت لوجبات الأسبوع! 💪 الاستمرارية هي سر النجاح. واصل!",
    "عمل رائع! الخطة الواضحة اليوم تعني توتراً أقل غداً.",
    "خطوة بخطوة تُبنى العادات الجيدة. خطتك مهمة!",
    "التخطيط هو الخطوة الأولى نحو النجاح. لقد أنجزت اليوم!",
    "كل وجبة مخططة هي خطوة نحو حياة متوازنة.",
    "اعتن بنفسك: التخطيط الجيد يعني صحة طويلة الأمد.",
    "انضباط اليوم هو حرية الغد. أحسنت!",
    "لقد حوّلت النوايا إلى أفعال. استمر!",
    "تُظهر خطتك الإصرار والاهتمام بنفسك وبأحبائك.",
    "الخطة الجيدة مثل الخريطة – توصلك مباشرةً إلى هدفك."
  ],
  zh: [
    "太棒了！你已完成本周所有餐食的计划！💪 持之以恒就是力量，继续加油！",
    "做得好！今天有清晰计划，明天更轻松。",
    "一步一步，好习惯就会养成。你的计划很重要！",
    "计划是成功的第一步。今天你做到了！",
    "每一餐的计划都是迈向均衡生活的一步。",
    "关爱自己：良好的计划意味着长期健康。",
    "今天的自律就是明天的自由。干得好！",
    "你把想法变成了行动。继续保持！",
    "你的计划展现了你的决心和对自己及家人的关心。",
    "好的计划就像地图——带你到达想去的地方。"
  ],
  ja: [
    "すばらしい！今週の食事計画はバッチリ！💪 継続は力なり。その調子！",
    "いいね！今日の明確な計画は、明日の負担を減らす。",
    "一歩ずつ、良い習慣は身につく。今日の計画が大切！",
    "計画は成功への第一歩。今日はやり遂げたね！",
    "計画された食事は、バランスの取れた生活への一歩。",
    "自分を大切に：良い計画は長期的な健康につながる。",
    "今日の規律は明日の自由。よくやった！",
    "意図を行動に変えたね。続けよう！",
    "あなたの計画は、あなたと大切な人への思いやりを示している。",
    "良い計画は地図のように—行きたい場所へ導いてくれる。"
  ],
  hi: [
    "शाबाश! आपने हफ्ते का पूरा मील प्लान बना लिया! 💪 निरंतरता ही ताकत है—जारी रखें!",
    "बहुत बढ़िया! आज की साफ़ योजना, कल का कम तनाव।",
    "एक-एक कदम, अच्छी आदतें बनती हैं। आपका प्लान मायने रखता है!",
    "योजना सफलता की पहली सीढ़ी है। आपने आज इसे पूरा किया!",
    "हर योजना बनाई गई भोजन एक संतुलित जीवन की ओर कदम है।",
    "अपना ख्याल रखें: अच्छी योजना का मतलब है लंबी अवधि की सेहत।",
    "आज का अनुशासन कल की आज़ादी है। बहुत बढ़िया!",
    "आपने इरादों को कार्य में बदला। इसे जारी रखें!",
    "आपकी योजना आपके और आपके प्रियजनों के लिए आपकी प्रतिबद्धता दिखाती है।",
    "अच्छी योजना एक नक्शे की तरह है—जो आपको सीधे आपके लक्ष्य तक ले जाती है।"
  ],
  tr: [
  "Tebrikler! Bu hafta tüm öğünlerini planladın! 💪 Disiplin, başarının anahtarıdır.",
  "Harika iş! Bugünün net planı, yarının daha az stresli olmasını sağlar.",
  "Adım adım, iyi alışkanlıklar oluşur. Bugünkü planın gerçekten önemli!",
  "Planlama başarının ilk adımıdır. Bugün bunu başardın!",
  "Her planlanan öğün, daha dengeli bir hayata doğru bir adımdır.",
  "Kendine iyi bak — iyi planlama uzun vadeli sağlık demektir.",
  "Bugünün disiplini, yarının özgürlüğüdür. Aferin!",
  "Niyetlerini eyleme dönüştürdün. Böyle devam et!",
  "Planların senin kararlılığını ve sevdiklerine olan özenini gösteriyor.",
  "İyi bir plan harita gibidir — seni tam olarak gitmek istediğin yere götürür."
],
it: [
  "Complimenti! Hai pianificato tutti i pasti della settimana! 💪 La costanza è la tua forza.",
  "Ottimo lavoro! Un piano chiaro oggi significa meno stress domani.",
  "Passo dopo passo, le buone abitudini si consolidano. Il tuo piano conta!",
  "La pianificazione è il primo passo verso il successo. Oggi ce l’hai fatta!",
  "Ogni pasto pianificato è un passo verso una vita più equilibrata.",
  "Abbi cura di te — una buona pianificazione significa salute a lungo termine.",
  "La disciplina di oggi è la libertà di domani. Ben fatto!",
  "Hai trasformato le intenzioni in azioni. Continua così!",
  "La tua pianificazione mostra determinazione e cura per te e i tuoi cari.",
  "Un buon piano è come una mappa — ti porta esattamente dove vuoi arrivare."
],
ko: [
  "멋져요! 이번 주 모든 식단을 계획했어요! 💪 꾸준함은 가장 강력한 힘입니다.",
  "좋아요! 오늘의 명확한 계획은 내일의 스트레스를 줄여줍니다.",
  "한 걸음씩 좋은 습관이 만들어집니다. 오늘의 계획은 정말 중요해요!",
  "계획은 성공의 첫 번째 단계입니다. 오늘 해냈어요!",
  "계획된 식사는 균형 잡힌 삶으로 가는 한 걸음입니다.",
  "스스로를 아끼세요 — 좋은 계획은 장기적인 건강으로 이어집니다.",
  "오늘의 규율은 내일의 자유입니다. 정말 잘했어요!",
  "당신은 의도를 행동으로 옮겼어요. 계속하세요!",
  "당신의 계획은 당신과 가족에 대한 배려와 결의를 보여줍니다.",
  "좋은 계획은 지도와 같습니다 — 원하는 곳으로 정확히 이끌어 줍니다."
],

}

const access = {
  ro: {
    validUntil: "Valabil până la",
    lifetime: "nelimitat"
  },
  en: {
    validUntil: "Valid until",
    lifetime: "unlimited"
  },
  es: {
    validUntil: "Válido hasta",
    lifetime: "ilimitado"
  },
  fr: {
    validUntil: "Valable jusqu'au",
    lifetime: "illimité"
  },
  de: {
    validUntil: "Gültig bis",
    lifetime: "unbegrenzt"
  },
  pt: {
    validUntil: "Válido até",
    lifetime: "ilimitado"
  },
  ru: {
    validUntil: "Действительно до",
    lifetime: "неограниченно"
  },
  ar: {
    validUntil: "صالح حتى",
    lifetime: "غير محدود"
  },
  zh: {
    validUntil: "有效期至",
    lifetime: "无限"
  },
  ja: {
    validUntil: "有効期限",
    lifetime: "無制限"
  },
  hi: {
    validUntil: "तक मान्य",
    lifetime: "असीमित"
  },tr: {
  validUntil: "Geçerlilik süresi",
  lifetime: "sınırsız"
},
it: {
  validUntil: "Valido fino al",
  lifetime: "illimitato"
},
ko: {
  validUntil: "유효 기간",
  lifetime: "무제한"
}
};

/* Cuisine-discovery homepage section strings. Single source of truth shared
   by the static generator (scripts/generate-content.mjs CUISINE_CTA reads
   these at build time) and the runtime localizer (app.js renderCuisineDiscover).
   {n} is replaced with the live cuisine count by both consumers. */
const CUISINE_I18N = {
  ro: { eyebrow:'Bucătării', heading:'Explorează bucătării din toată lumea', sub:'{n} bucătării internaționale, fiecare cu rețete autentice și planificator gratuit.', btn:'Vezi toate cele {n} bucătării' },
  en: { eyebrow:'Cuisines', heading:'Explore world cuisines', sub:'{n} international cuisines, each with authentic recipes and a free meal planner.', btn:'Browse all {n} cuisines' },
  es: { eyebrow:'Cocinas', heading:'Explora cocinas del mundo', sub:'{n} cocinas internacionales, cada una con recetas auténticas y planificador gratuito.', btn:'Ver las {n} cocinas' },
  fr: { eyebrow:'Cuisines', heading:'Explorez les cuisines du monde', sub:'{n} cuisines internationales, chacune avec des recettes authentiques et un planificateur gratuit.', btn:'Voir les {n} cuisines' },
  de: { eyebrow:'Küchen', heading:'Entdecke Küchen aus aller Welt', sub:'{n} internationale Küchen, jede mit authentischen Rezepten und kostenlosem Planer.', btn:'Alle {n} Küchen ansehen' },
  pt: { eyebrow:'Cozinhas', heading:'Explore cozinhas do mundo', sub:'{n} cozinhas internacionais, cada uma com receitas autênticas e planejador gratuito.', btn:'Ver as {n} cozinhas' },
  ru: { eyebrow:'Кухни', heading:'Изучите кухни мира', sub:'{n} мировых кухонь — подлинные рецепты и бесплатный планировщик меню.', btn:'Все {n} кухни' },
  ar: { eyebrow:'مطابخ', heading:'اكتشف مطابخ العالم', sub:'{n} مطبخًا عالميًا، كل منها بوصفات أصيلة ومخطط وجبات مجاني.', btn:'تصفح جميع المطابخ الـ {n}' },
  zh: { eyebrow:'菜系', heading:'探索世界各国菜系', sub:'{n}个世界菜系，每个都有正宗菜谱和免费每周饮食计划。', btn:'浏览全部{n}个菜系' },
  ja: { eyebrow:'料理', heading:'世界の料理を探す', sub:'{n}か国の世界料理。本格レシピと無料の週間ミールプランナー。', btn:'{n}か国すべて見る' },
  hi: { eyebrow:'व्यंजन', heading:'दुनिया के व्यंजन देखें', sub:'{n} वैश्विक व्यंजन, हर एक प्रामाणिक रेसिपी और मुफ्त मील प्लानर के साथ।', btn:'सभी {n} व्यंजन देखें' },
  tr: { eyebrow:'Mutfaklar', heading:'Dünya mutfaklarını keşfedin', sub:'{n} dünya mutfağı — otantik tarifler ve ücretsiz öğün planlayıcı.', btn:'Tüm {n} mutfağı görüntüle' },
  it: { eyebrow:'Cucine', heading:'Esplora le cucine del mondo', sub:'{n} cucine internazionali, ognuna con ricette autentiche e pianificatore gratuito.', btn:'Vedi tutte le {n} cucine' },
  ko: { eyebrow:'요리', heading:'세계 요리를 탐색하세요', sub:'{n}개국 세계 요리 — 정통 레시피와 무료 주간 식단 플래너.', btn:'{n}개 요리 모두 보기' },
};
for (const lc in CUISINE_I18N) {
  if (!i18n[lc]) continue;
  const c = CUISINE_I18N[lc];
  i18n[lc]['cuisine.eyebrow'] = c.eyebrow;
  i18n[lc]['cuisine.heading'] = c.heading;
  i18n[lc]['cuisine.sub']     = c.sub;
  i18n[lc]['cuisine.btn']     = c.btn;
}

export { i18n, langNames, seoParagraphs, pdfMessages, MOTIV,access };



...
npm run content       # regenerates 2562 HTML pages + public/sitemap.xml
...
- HTML page count is 2560–2650 (target 2562)
- `public/sitemap.xml` has 2600–2700 `<url>` entries (target 2605)
...
2. **Any edit to `public/js/recipes.js` requires `npm run content`** to regenerate HTML, and the resulting page count must stay ~2562.
...
`scripts/generate-content.mjs` is the engine. It imports `public/js/recipes.js`, `public/js/recipes-budget.js`, and `public/js/i18n.js`, then writes ~2562 static HTML pages:
